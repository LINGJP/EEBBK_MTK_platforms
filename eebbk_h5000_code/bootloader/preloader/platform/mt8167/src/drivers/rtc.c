...
static U16 get_frequency_meter(U16 val, U16 measureSrc, U16 window_size)
{
	...
	RTC_Write(MT6392_FQMTR_CON0, 0x0000); //disable freq. meter, gated src clock
	RTC_Write(MT6392_TOP_CKPDN1, RTC_Read(MT6392_TOP_CKPDN1) | RG_FQMTR_PDN); //TOP_CKPDN1[5]=1, gated fixed clock
	RTC_Write(MT6392_TOP_RST_CON, RTC_Read(MT6392_TOP_RST_CON) | RG_FQMTR_RST);			//FQMTR reset
	while( !(RTC_Read(MT6392_FQMTR_CON2)==0) && (FQMTR_BUSY&RTC_Read(MT6392_FQMTR_CON0))==FQMTR_BUSY);
	RTC_Write(MT6392_TOP_RST_CON, RTC_Read(MT6392_TOP_RST_CON) & ~RG_FQMTR_RST);			//FQMTR normal
	RTC_Write(MT6392_TOP_CKPDN1, RTC_Read(MT6392_TOP_CKPDN1) & ~RG_FQMTR_PDN); //TOP_CKPDN1[5]=0, turn on fixed clock

	RTC_Write(MT6392_FQMTR_CON1, window_size); //set freq. meter window value (0=1X32K(fix clock))
	RTC_Write(MT6392_FQMTR_CON0, FQMTR_EN | measureSrc); //enable freq. meter, set measure clock to 26Mhz

	mdelay(1);
	while( (FQMTR_BUSY&RTC_Read(MT6392_FQMTR_CON0))==FQMTR_BUSY )
	{
	...
	ret = RTC_Read(MT6392_FQMTR_CON2);				//read data should be closed to 26M/32k = 812.5		
...
static void rtc_measure_four_clock(U16 *result)
{
	...
	RTC_Write(MT6392_TOP_CKCON1, (RTC_Read(MT6392_TOP_CKCON1) & FQMTR_FIX_CLK_26M) ); //select 26M as fixed clock
	window_size = 4;
	mdelay(1);
	result[0] = get_frequency_meter(0, FQMTR_AUD26M_CK, window_size); 		//select 26M as target clock
	
	//select XOSC_DET as fixed clock  
	RTC_Write(MT6392_TOP_CKCON1, (RTC_Read(MT6392_TOP_CKCON1) | FQMTR_FIX_CLK_XOSC_32K_DET));
	window_size = 4;
	mdelay(1);
	result[1] = get_frequency_meter(0, FQMTR_AUD26M_CK, window_size); 		//select 26M as target clock
	
	//select 26M as fixed clock
	RTC_Write(MT6392_TOP_CKCON1, (RTC_Read(MT6392_TOP_CKCON1) & FQMTR_FIX_CLK_26M));
	...
	RTC_Write(MT6392_TOP_CKCON1, (RTC_Read(MT6392_TOP_CKCON1) | FQMTR_FIX_CLK_EOSC_32K));
	window_size = 4;
	mdelay(1);
...
static void rtc_switch_mode(bool XOSC, bool recovery)
{
	if (XOSC)
	{
		if (recovery)
		{
			...
			RTC_Write(MT6392_CHRSTATUS, (RTC_Read(MT6392_CHRSTATUS) | RTC_HW_DET_BYPASS & RTC_HW_XOSC_MODE));
		}
	...
	} else 
	{
		if (recovery)
		{
			...
			RTC_Write(MT6392_CHRSTATUS, (RTC_Read(MT6392_CHRSTATUS) | RTC_HW_DET_BYPASS | RTC_HW_DCXO_MODE));
		}
...
static bool rtc_init_after_recovery(void)
{
	...
	RTC_Write(MT6392_CHRSTATUS, (RTC_Read(MT6392_CHRSTATUS) & ~0x0800));
...
static U16 eosc_cali(void)
{
	...
	RTC_Write(MT6392_TOP_CKCON1, (RTC_Read(MT6392_TOP_CKCON1) | FQMTR_FIX_CLK_EOSC_32K) );//fix clock = eosc 32K
	print("[RTC] EOSC_Cali: TOP_CKCON1=0x%x\n", RTC_Read(MT6392_TOP_CKCON1));
	while( left<=(right) )
	{
		middle = (right + left) / 2;
		if(middle == left)
...
int rtc_boot_check(void)
{
	U16 irqsta, pdn1, pdn2, spar0, spar1, Rdata, al_dom;
	U16 result[4];
	U16 XOSC = 0;
	U16 cali;
	...
	RTC_Write(MT6392_TOP_CKPDN1, 0x3df);
	...
	RTC_Write(MT6392_TOP_CKTST2, RTC_Read(MT6392_TOP_CKTST2) | 0x0110);
	
	RTC_Write(MT6392_TOP_CKPDN1, 0x3ff);
	rtc_clean_mark();
	...
	al_dom = RTC_Read(RTC_AL_DOM);
	print("[RTC] irqsta = 0x%x, pdn1 = 0x%x, pdn2 = 0x%x, spar0 = 0x%x, spar1 = 0x%x, al_dom = 0x%x\n",
		  irqsta, pdn1, pdn2, spar0, spar1, al_dom);
	...
	Write_trigger();
	print("[RTC] spar0 = 0x%x\n", RTC_Read(RTC_SPAR0));

	if ((irqsta & RTC_IRQ_STA_AL) | (spar0 & RTC_SPAR0_ALARM_BOOT)) {
#if RTC_RELPWR_WHEN_XRST
	...
	if ((pdn1 & RTC_PDN1_RECOVERY_MASK) == RTC_PDN1_FAC_RESET) {	/* factory data reset */
		/* keep bit 4 set until rtc_boot_check() in LK */
		print("[RTC] RTC_REBOOT_RECOVERY\n");
		return RTC_REBOOT_RECOVERY;
	}
	/* check bootloader, panic/restart/aee here*/
	if ((pdn1 & 0x2000) == 0x2000) {
		print("[RTC] RTC_REBOOT_BOOTLOADER\n");
		return RTC_REBOOT_BOOTLOADER;
	}

	if ((al_dom & 0x0400) == 0x0400) {
		print("[RTC] RTC_KERNEL_AEE\n");
		return RTC_KERNEL_AEE;
	} else if ((al_dom & 0x0100) == 0x0100) {
		print("[RTC] RTC_KERNEL_PANIC\n");
		return RTC_KERNEL_PANIC;
	} else if ((al_dom & 0x0200) == 0x0200) {
		print("[RTC] RTC_KERNEL_RESTART\n");
		return RTC_KERNEL_RESTART;
	}
