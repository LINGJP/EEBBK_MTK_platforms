 ...
 * 05/30/12 YC		Modify AP3220C_check_and_clear_intr return value and exchange
 *					AP3220C_get_ps_value return value to meet our spec.
 * 05/30/12 YC		Correct shift number in AP3220C_read_ps.
...
#define NVRAM_IOCTL_SET_HOR_LIGHT  _IOW('C', 0x01, unsigned int[3])
#define NVRAM_IOCTL_SET_VER_LIGHT  _IOW('C', 0x02, unsigned int[3])
#define NVRAM_IOCTL_SET_HOR_PS  _IOW('C', 0x03, unsigned int[2])
#define NVRAM_IOCTL_SET_VER_PS  _IOW('C', 0x04, unsigned int[2])
#define NVRAM_IOCTL_GET_CAIL  _IOR('C', 0x05, unsigned int[2])
#define NOT_USEFUL  _IOR('C', 0x06, unsigned int[1])
...
#else
	#define AP3XX6_DEV_NAME	"AP3220"
	#define AP3XX6_DEV_NAME_vertical "AP3220_vertical"
#endif

/*----------------------------------------------------------------------------*/
/* ap3xx6_obj is current sensor, ap3xx6_obj_vertical is i2c_addr is 38,ap3xx6_obj_horizontal is i2c_addr is horizontal*/
/*-----------------------------------------------------------------------------*/
static struct i2c_client *ap3xx6_i2c_client = NULL;

static struct ap3xx6_priv *ap3xx6_obj = NULL;

static struct i2c_client *ap3xx6_i2c_client_horizontal = NULL;//add anthor alsps

static struct ap3xx6_priv *ap3xx6_obj_horizontal = NULL;

static struct i2c_client *ap3xx6_i2c_client_vertical = NULL;

static struct ap3xx6_priv *ap3xx6_obj_vertical = NULL;


/*----------------------------------------------------------------------------*/
static const struct i2c_device_id ap3xx6_i2c_id[] = {{AP3XX6_DEV_NAME, 0}, {} };
static const struct i2c_device_id ap3xx6_i2c_id_vertical[] = {{AP3XX6_DEV_NAME_vertical, 0}, {} };
...
/*----------------------------------------------------------------------------*/
/********this is alsps sensor of the gpio of 38**********************************/
static int ap3xx6_i2c_probe_vertical(struct i2c_client *client, const struct i2c_device_id *id);
static int ap3xx6_i2c_remove_vertical(struct i2c_client *client);
static int ap3xx6_i2c_detect_vertical(struct i2c_client *client, struct i2c_board_info *info);
/*----------------------------------------------------------------------------*/
...
/*add by lyq*/
extern struct current_sensor_s *current_sensor;
...
static const struct of_device_id alsps_of_match_vertical[] = {
	{.compatible = "mediatek,alsps_vertical"},
	{},
};
...
static struct i2c_driver ap3xx6_i2c_driver_vertical = {
	.probe		= ap3xx6_i2c_probe_vertical,
	.remove	= ap3xx6_i2c_remove_vertical,
	.detect = ap3xx6_i2c_detect_vertical,
	//.suspend	= ap3xx6_i2c_suspend,
	//.resume	= ap3xx6_i2c_resume,
	.id_table	= ap3xx6_i2c_id_vertical,
	.driver = {
/* .owner			= THIS_MODULE, */
		.name			= AP3XX6_DEV_NAME_vertical,
#ifdef CONFIG_OF
		.of_match_table = alsps_of_match_vertical,
#endif
	},
};
...
static int ap3xx6_init_flag = -1; /* 1<==>hor OK, 2 ver ok ,3 both ok -1 <==> fail */
...
{
	printk("lyq:entry %s\n",__func__);
	if(ap3xx6_obj_horizontal != NULL)
		enable_irq(ap3xx6_obj_horizontal->irq);
	if(ap3xx6_obj_vertical != NULL)
		enable_irq(ap3xx6_obj_vertical->irq);	
}/*
void disable_two_irq(void)
{
	printk("lyq:entry %s\n",__func__);
	if(ap3xx6_obj_horizontal != NULL)
		disable_irq_nosync(ap3xx6_obj_horizontal->irq);
	if(ap3xx6_obj_vertical != NULL)
		disable_irq_nosync(ap3xx6_obj_vertical->irq);
}*/

int ap3xx6_read_reg(struct i2c_client *client,
...
/*	#if (!DELAYED_WORK)		add by lyq to solve psensor no enable irq question
...
#else
/*val=0x00~0xF*/
	re_val= ap3xx6_write_reg(client, 0x10, 0xFF, 0x00, val);
#endif
...
void ap3xx6_eint_func(struct ap3xx6_priv *obj)
{
...
static irqreturn_t ap3xx6_eint_handler(int irq, void *desc)
{
	...
	ap3xx6_eint_func(obj);
	
	return IRQ_HANDLED;
}
static irqreturn_t ap3xx6_eint_ver_handler(int irq, void *desc)
{
        struct ap3xx6_priv *obj = NULL;

        printk("lyq:entry %s\n",__func__);
	disable_irq_nosync(ap3xx6_obj_vertical->irq);
        obj = ap3xx6_obj_vertical;
...
int ap3xx6_setup_eint(struct i2c_client *client)
{
	...
	if (!ap3xx6_obj_horizontal) {
		APS_ERR("ap3xx6_obj_horizontal is null!!\n");
		return 0;
	}

#if defined(CONFIG_OF)
	if (ap3xx6_obj_horizontal->irq_node) {
		//of_property_read_u32_array(ap3xx6_obj->irq_node, "debounce", ints, ARRAY_SIZE(ints));
		ints[0]=of_get_named_gpio(ap3xx6_obj_horizontal->irq_node,"ap3220-int-gpio", 0);
		ints[1]=256000;	
		
		gpio_request(ints[0], "p-sensor");	
		gpio_direction_input(ints[0]);
		
		gpio_set_debounce(ints[0], ints[1]);
		
		APS_LOG("ints[0]=%d, ints[1]=%d\n", ints[0], ints[1]);
	
		ap3xx6_obj_horizontal->irq = irq_of_parse_and_map(ap3xx6_obj_horizontal->irq_node, 0);
	
		APS_LOG("ap3xx6_obj_horizontal->irq = %d\n", ap3xx6_obj_horizontal->irq);
		if (!ap3xx6_obj_horizontal->irq) {
			APS_ERR("irq_of_parse_and_map fail!!\n");
			return -EINVAL;
		}
		err = request_irq(ap3xx6_obj_horizontal->irq, ap3xx6_eint_handler, IRQF_TRIGGER_NONE, "ALS-eint", NULL);
		if (err != 0) {
...
int ap3xx6_setup_eint_vertical(void)
{
	u32 ints[2] = {0};
	int err = 0;

	if (!ap3xx6_obj_vertical) {
		APS_ERR("ap3xx6_obj_horizontal is null!!\n");
		return 0;
	}
	
	if (ap3xx6_obj_vertical->irq_node) {
		//of_property_read_u32_array(ap3xx6_obj->irq_node, "debounce", ints, ARRAY_SIZE(ints));
		ints[0]=of_get_named_gpio(ap3xx6_obj_vertical->irq_node,"ap3220-int-gpio_vertical", 0);
		ints[1]=256000;	
		
		gpio_request(ints[0], "p-sensor_vertical");	
		gpio_direction_input(ints[0]);
		gpio_set_debounce(ints[0], ints[1]);
		
		APS_LOG("ints[0]=%d, ints[1]=%d\n", ints[0], ints[1]);
	
		ap3xx6_obj_vertical->irq = irq_of_parse_and_map(ap3xx6_obj_vertical->irq_node, 0);
	
		APS_LOG("ap3xx6_obj_vertical->irq = %d\n", ap3xx6_obj_vertical->irq);
		if (!ap3xx6_obj_vertical->irq) {
			APS_ERR("irq_of_parse_and_map fail!!\n");
			return -EINVAL;
		}
		err = request_irq(ap3xx6_obj_vertical->irq, ap3xx6_eint_ver_handler, IRQF_TRIGGER_NONE, "ALS-eint-vertical", NULL);
...
static int ap3xx6_init_client(struct i2c_client *client)
{
	...
	/****************lyq just ps test****************/
	val = (AP3220_PS_INTG_TIME_16 << AP3220_PS_INTG_SHIFT)
		| (AP3220_PS_GAIN_4 << AP3220_PS_GAIN_SHIFT)
		| (AP3220_PS_PERSIST_2 << AP3220_PS_PERSIST_SHIFT);
	printk("val_config = %d\n", val);
	res = ap3xx6_write_reg(client, AP3220_PS_CONFIG, 0xFF, 0x00, val);

	val = (AP3220_PS_LED_PULSE_3 << AP3220_LED_PULSE_SHIFT)
		| (AP3220_PS_LED_RATIO_66 << AP3220_LED_RATIO_SHIFT);		//change from 33 to 66 by lyq for H5000 psensor
	printk("val_ctrl = %d\n", val);
	res = ap3xx6_write_reg(client, AP3220_LED_CTRL, 0xFF, 0x00, val);

	val = AP3220_PS_INT_ZONE << AP3220_PS_INT_SHIFT;	
	res = ap3xx6_write_reg(client, AP3220_PS_INT_MODE, 0xFF, 0x00, val);

	ap3xx6_set_meantime(client, 0x02);
	
	val = AP3220_LED_WAIT_2 << AP3220_LED_WAIT_SHIFT;
	printk("led waiting time = %d\n", val);
	res = ap3xx6_write_reg(client, AP3220_LED_WAITING, 0xFF, 0x00, 0x02);
...
/****************************light sensor start****************************/
	...
	printk("lyq:  psthd_val_l is %d, h is %d\n",atomic_read(&obj->ps_thd_val_l),atomic_read(&obj->ps_thd_val_h));
	
	ap3xx6_set_ALSGain(client, LS_LUX_1023);

	for(i=0; i< obj->als_level_num;i++)
	{
		APS_LOG("lyq:obj->als_level[%d] is %d", i, obj->als_level[i]);
		APS_LOG("lyq:obj->als_value[%d] is %d", i, obj->als_value[i]);	
	}
...
#endif
//	APS_LOG("lyq:the ps adc value is %d\n",*data);
	return 0;
...
static int ap3xx6_get_ps_value(struct ap3xx6_priv *obj, u16 ps)
{
	...
	printk("lyq: the psdata is %d, thd_val_h is %d, thd_val_l is %d\n",ps,atomic_read(&obj->ps_thd_val_h),atomic_read(&obj->ps_thd_val_l));
...
static int ap3xx6_Calibration_every_time(struct i2c_client *client)
{
	...
	if(ps_data_avg > 511)
	{
		ap3xx6_set_pcrosstalk(obj->client, 511);
	}
	else
	{
		ap3xx6_set_pcrosstalk(obj->client, ps_data_avg);
	}
	msleep(70);
...
static int ap3xx6_enable_ps_interrupt(struct i2c_client *client, int enable)
{
//	struct ap3xx6_priv *obj = i2c_get_clientdata(client);
	u8 buffer[1];
	int res = 0;
	...
	if(enable)
	{
		databuf[0] = AP3xx6_INT_CTL;
		databuf[1] = reg_value[0] | 0x80;
		res = ap3xx6_write_reg(client, databuf[0], 0xFF, 0x00, databuf[1]);
		if (res <= 0) 
		{
			goto EXIT_ERR;
...
EXIT_ERR:
	APS_ERR("ap3xx6_enable_ps_interrupt fail\n");
	return res;
...
static void ap3xx6_eint_work(struct work_struct *work)
{
	...
	if(err < 0)
	{
		...
		ps_val_level = ap3220_change_ps_threshold(obj->client);
		err = ps_report_interrupt_data(ps_val_level);
...
static ssize_t ap3xx6_status_show(struct device_driver *ddri, char *buf)
{
	...
	for (i = 0; i < sizeof(obj->als_level)/sizeof(obj->als_level[0]); i++) {
		ret += sprintf(buf+ret, "%d, ", obj->als_level[i]);
	}
	ret += sprintf(buf+ret, "\n als_value:");
	for (i = 0; i < sizeof(obj->als_value)/sizeof(obj->als_value[0]); i++) {
		ret += sprintf(buf+ret, "%d, ", obj->als_value[i]);
	}
...
static long ap3xx6_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	int dat;
	uint32_t enable;
	unsigned int ps_cali_temp[2];	//0 is flag, 1 is 25cm
	unsigned int ls_cali_temp[3];	//0 is flag, 1 dark, 2 normal
	...
	case NVRAM_IOCTL_GET_CAIL:
			...
			break;
	case NOT_USEFUL:
		APS_ERR("%s error Should not be called,just for Solve compilation problems\n",__func__);	
		set_ver_ps_cail(ps_cali_temp);
		set_hor_light_cail(ls_cali_temp);	
			break;
/*
	case NVRAM_IOCTL_SET_VER_PS:
			if(copy_from_user(ps_cali_temp, ptr, sizeof(ps_cali_temp)))
			{
				err = -EFAULT;
				goto err_out;
			}
			if(set_ver_ps_cail(ps_cali_temp))
				break;
			else
				APS_ERR("%s set ver cail is failure\n",__func__);
			break;
*/	
	case NVRAM_IOCTL_SET_HOR_PS:
			if(copy_from_user(&ps_cali_temp, ptr, sizeof(ps_cali_temp)))//数组和指针的关系
			{
				err = -EFAULT;
				goto err_out;
			}
			if(set_hor_ps_cail(ps_cali_temp))
				break;
			else
				APS_ERR("%s set hor cail is failure\n",__func__);
			break;

/*	case NVRAM_IOCTL_SET_VER_LIGHT:
			if(copy_from_user(ls_cali_temp, ptr, sizeof(ls_cali_temp)))//数组和指针的关系
			{
				err = -EFAULT;
				goto err_out;
			}
			if(set_ver_light_cail(ls_cali_temp))
				break;
			else
				APS_ERR("%s set light ver cail is failure\n",__func__);
			break;
*/	
	case NVRAM_IOCTL_SET_HOR_LIGHT:
			if(copy_from_user(ls_cali_temp, ptr, sizeof(ls_cali_temp)))//数组和指针的关系
			{
				err = -EFAULT;
				goto err_out;
			}
			if(set_hor_light_cail(ls_cali_temp))
				break;
			else
				APS_ERR("%s set light hor cail is failure\n",__func__);
			break;
...
/*白炽灯下会干扰，导致出现误报最暗的情况，这个函数，就是检测这种情况的
返回值:1是误报的情况，不上报。0，确实是最暗条件，上报。
*/
int is_als_should_report(unsigned int als_value, struct ap3xx6_priv *obj)
{
	unsigned int als_value_low[0],als_value_high[0];

	if((als_value == (obj->als_value[1])) && (current_sensor->sensor_pls))
	{
		als_value_low[0] = ap3xx6_read_reg(obj->client, 0x0A, 0xFF, 0x00);
        	if (als_value_low[0] < 0) {
                	return 0;
        	}

        	als_value_high[0] = ap3xx6_read_reg(obj->client, 0x0B, 0xFF, 0x00);
        	if (als_value_high[0] < 0) {
                	return 0;
	       }
        
		als_value_high[0] = ((als_value_high[0]<<2)|(als_value_low[0]&0x03));
	
		if(als_value_high[0] >100)
    	        {
			printk("lyq:%s error read als ir_reg and the ir value is %d\n",__func__,als_value_high[0]);
       			 return 1;
		}
		else
			return 0;
...
static int ap3xx6_als_get_data(int *value, int *status)
{
...
#endif
	{
		...
		obj->als = (b[0] + b[1] + obj->als)/3;
		
//		obj->als = (u16)change_als_lux(obj->client,obj->als);	//change lux lange, unuseful so //
	...
	*status = SENSOR_STATUS_ACCURACY_MEDIUM;
	...
	if(is_als_should_report(temp_value, obj))
		return 1;	
	
	if(temp_als != temp_value)
	{	
		temp_als = temp_value;
		APS_LOG("lyq: als_raw data is %d,ALS input value is %d\n",obj->als,temp_value);
	}
...
*/
		...
		err = ap3xx6_enable_ps_interrupt(obj->client, 1);
		if (err != 0)
		{
			APS_ERR(" ap3xx6 enable ps interrupt fail: %d\n", err);
			return -1;
		}
		msleep(50);
#else
	err = ap3xx6_enable_ps(obj->client, 1);
		...
		ps_val_level= get_ps_level(obj->client,0xff);
		ps_report_interrupt_data(ps_val_level);
	} else{
		err = ap3xx6_enable_ps(obj->client, 0);
		if (err != 0) {
		...
		clear_bit(CMC_BIT_PS, &obj->enable);
#ifdef DI_AUTO_CAL
		err = ap3xx6_enable_ps_interrupt(obj->client, 0);
		if (err != 0)
		{
			APS_ERR(" ap3xx6 disable ps interrupt fail: %d\n", err);
			return -1;
		}
#endif
...
/*-----------------------for both sensor work start ----------------------------------*/
/*
int set_pins_for_both(void)
{
	struct pinctrl *pinctrl;
	struct pinctrl_state *pins_default;
	struct pinctrl_state *pins_cfg;
	struct pinctrl_state *pins_cfg_vertical;
	int ret=0;
	...
	pins_cfg_vertical = pinctrl_lookup_state(pinctrl, "pin_cfg_vertical");	// no need vertical alsps
	if (IS_ERR(pins_cfg_vertical)) {
		ret = PTR_ERR(pins_cfg_vertical);
		APS_ERR("Cannot find alsps pinctrl pin_cfg_vertical!\n");
		return ret;
		}
	else
		pinctrl_select_state(pinctrl, pins_cfg_vertical);
#else
...
int register_all_sysfs(void)
{
	...
	ps_ctl.is_support_batch = false;//lyq
	...
	als_ctl.is_support_batch = false;//lyq obj->hw->is_batch_supported_als;
...
void chose_defaul_sensor(void)
{

	if (i2c_add_driver(&ap3xx6_i2c_driver)) {
		APS_ERR("add horizontal sensor error\n");
	}//must befor vertical i2c_add
	if (i2c_add_driver(&ap3xx6_i2c_driver_vertical)) {
		APS_ERR("add vertical sensor error\n");
	}
	
	if((ap3xx6_init_flag==SENSOR_BOTH_OK) ||(ap3xx6_init_flag==SENSOR_HOR_OK))
		{
			register_all_sysfs();
			ap3xx6_obj=ap3xx6_obj_horizontal;
			ap3xx6_i2c_client = ap3xx6_i2c_client_horizontal;
		}
	else if(ap3xx6_init_flag==SENSOR_VER_OK)
		{
			register_all_sysfs();
			ap3xx6_obj=ap3xx6_obj_vertical;
			ap3xx6_i2c_client = ap3xx6_i2c_client_vertical;
		}
	else
		{
			ap3xx6_obj=NULL;
...
{
	return ap3xx6_init_flag;
}
/*-----------------------for both sensor work end ----------------------------------*/
...
static int ap3xx6_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ap3xx6_obj_horizontal = obj;
	ap3xx6_obj=ap3xx6_obj_horizontal;//probe need ap3xx6_obj do some work
	...
	/*hor的光感最大值比较难上去，所以将最值触发调小*/
	obj->als_level[6] = 10000;
	obj->als_level[7] = 18000;
	obj->als_level[8] = 34000;
		
	BUG_ON(sizeof(obj->als_value) != sizeof(obj->hw->als_value));
	memcpy(obj->als_value, obj->hw->als_value, sizeof(obj->als_value));
	atomic_set(&obj->i2c_retry, 3);

	ap3xx6_i2c_client_horizontal = client;
	...
	if (1 == obj->hw->polling_mode_ps) {
		...
		ps_ctl.is_polling_mode = true;*///lyq add for interrupt mode
	...
	ap3xx6_init_flag = SENSOR_HOR_OK;
	APS_LOG("%s: OK\n", __func__);
...
exit_init_failed:
	mutex_destroy(&ap3xx6_obj_horizontal->lock);
	kfree(obj);
exit:
	ap3xx6_init_flag = SENSOR_HOR_FAILED;
	
	ap3xx6_i2c_client_horizontal = NULL;
	ap3xx6_obj=NULL;
	ap3xx6_obj_horizontal = NULL;
	APS_ERR("%s: err = %d\n", __func__, err);
	return err;
}
/*----------------------------------------------------------------------------*/
static int ap3xx6_i2c_remove(struct i2c_client *client)
{
	...
	if (1 == ap3xx6_obj_horizontal->hw->polling_mode_ps) {
	...
	kfree(ap3xx6_i2c_client_horizontal);
        ap3xx6_i2c_client_horizontal = NULL;
...
static int ap3xx6_i2c_probe_vertical(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	memset(obj, 0, sizeof(*obj));
	ap3xx6_obj_vertical = obj;
	ap3xx6_obj=ap3xx6_obj_vertical;//init need pointer
	...
	ap3xx6_get_addr(obj->hw, &obj->addr);
...
#if defined(CONFIG_OF)
		obj->irq_node = of_find_compatible_node(NULL, NULL, "mediatek, als-eint_vertical");
#endif
		...
		ap3xx6_i2c_client_vertical = client;
		...
		if (1 == obj->hw->polling_mode_ps) {
			...
			err = ap3xx6_setup_eint_vertical();
			if (err != 0) {
				APS_ERR("setup eint: %d\n", err);
				goto exit_create_attr_failed;
			}
		...
		register_early_suspend(&obj->early_drv);
#endif
		if(ap3xx6_init_flag == SENSOR_HOR_OK)
			ap3xx6_init_flag=SENSOR_BOTH_OK;
		else
			ap3xx6_init_flag=SENSOR_VER_OK;
		APS_LOG("%s: OK\n", __func__);

		return 0;
	
	exit_create_attr_failed:
		printk("ALS/PS:the location is probe end\n");
	exit_init_failed:
		mutex_destroy(&ap3xx6_obj_vertical->lock);
	exit:	
		kfree(ap3xx6_obj_vertical);
		ap3xx6_obj_vertical=NULL;
		ap3xx6_obj=NULL;

		ap3xx6_i2c_client_vertical = NULL;
		if(ap3xx6_init_flag == SENSOR_HOR_FAILED)
			ap3xx6_init_flag = SENSOR_BOTH_FAILED;
...
static int ap3xx6_i2c_remove_vertical(struct i2c_client *client)
{
	...
	if (1 == ap3xx6_obj_vertical->hw->polling_mode_ps) {
		wake_lock_destroy(&chrg_lock);
	}
...
int change_sensor_ic(int buf)
{
	if(buf==1)
	{
		...
		{
		APS_ERR("ap3xx6_obj_vertical is null\n");
		return 0;
		}
	}
	else if (buf==0)
	{
		...
		printk("lyq:%s is faile\n",__func__);
	return 1;
}
/****************light sensor cail func****************/
int set_ver_light_cail(unsigned int *p)
{
	unsigned int flag =0;
	unsigned int ls_dark,ls_normal;
	int i;
	struct ap3xx6_priv *obj = ap3xx6_obj_vertical;

	if(obj==NULL)
	{
		APS_ERR("lyq:ap3xx6_obj_vertical is NULL\n");
		return -1;
	}
	...
	APS_LOG("lyq:%s the ver light cail flag is %d, dark is %d, cail nornal is %d\n",__func__,flag,ls_dark,ls_normal);
	
	if(flag ==0||ls_dark>10)
	{
		APS_ERR("lyq: %s faile\n",__func__);
		return 0;
	}
	...
	for(i=2; i<10; i++)
	{
		if(ls_normal > ((STANDARD_LIGHT_VALUE_VER*12)/10))
		{
			obj->als_level[i] = (obj->als_level[i] *12)/10;
		}
		else if(ls_normal < ((STANDARD_LIGHT_VALUE_VER*8)/10))
		{
...
int set_hor_light_cail(unsigned int *p)
{
	...
	int i;
	unsigned int ls_dark,ls_normal;
	struct ap3xx6_priv *obj = ap3xx6_obj_horizontal;
	...
	APS_LOG("lyq:%s the hor light cail flag is %d, dark is %d, cail nornal is %d\n",__func__,flag,ls_dark,ls_normal);
	...
	for(i=2; i<10; i++)
	{
		if(ls_normal > ((STANDARD_LIGHT_VALUE_HOR*12)/10))
		{
			obj->als_level[i] = (obj->als_level[i] *12)/10;
		}
		else if(ls_normal < ((STANDARD_LIGHT_VALUE_HOR*8)/10))
		{
			obj->als_level[i] = (obj->als_level[i] *8)/10;
		}
		else
...
static int ap3xx6_i2c_detect_vertical(struct i2c_client *client, struct i2c_board_info *info)
{
	strcpy(info->type, AP3XX6_DEV_NAME_vertical);
	return 0;
...
static int ap3xx6_local_init(void)
{
	...
	ap3xx6_power(hw, 1);
	//set_pins_for_both();	//Dynamic Modify pin mode
	chose_defaul_sensor();
...
{
	const char *name = "mediatek,AP3220";
	APS_FUN();
	printk("lyq: this is %s\n",__func__);
