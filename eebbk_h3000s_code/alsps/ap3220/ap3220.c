...
int write_ps_ctr_reg(struct i2c_client *client, int enable)
{
	u8 databuf[2];
        int res = 0;
	u8 buffer[1];
...
static int ap3xx6_enable_ps(struct i2c_client *client, int enable)
{
	...
	if (enable) {
		...
		if (0 == obj->hw->polling_mode_ps) {
/*
	#if (!DELAYED_WORK)					add by lyq to solve psensor no enable_irq
...
/*val=0x00~0x03*/
	...
	LYQ_LOG("reg 0x23 is %d\n", re_val);
	return err;
}
static int ap3xx6_init_als(struct i2c_client *client)
{
	int re_val, err;
...
int ap3220_ps_init(struct i2c_client *client)
{
	struct ap3xx6_priv *obj = i2c_get_clientdata(client);
	int res = 0;
        u8 val;
...
static irqreturn_t ap3xx6_eint_handler(int irq, void *desc)
{
	...
	LYQ_LOG("Entry ap3xx6_eint_handler\n");	
	if (unlikely(obj == NULL)) {
		APS_ERR("%s--%d ap3xx6_obj is NULL!\n", __func__, __LINE__);
		return IRQ_HANDLED;
	}
...
int ap3xx6_setup_eint(struct i2c_client *client)
{
	...
	pins_cfg = pinctrl_lookup_state(pinctrl, "pin_cfg");	//lyq no select state
	if (IS_ERR(pins_cfg)) {
	...
	if (ap3xx6_obj->irq_node) {
		...
		ints[0]=of_get_named_gpio(ap3xx6_obj->irq_node, "int-gpio", 0);
		ints[1] = 256000;
	
		gpio_request(ints[0], "p-sensor");	
//		gpio_set_debounce(ints[0], ints[1]);	add by lyq to solve interrupt abnormity
		...
		err = request_irq(ap3xx6_obj->irq, ap3xx6_eint_handler, IRQ_TYPE_EDGE_FALLING, "ALS-eint", NULL);
...
/*----------------------------------------------------------------------------*/
static int ap3xx6_init_client(struct i2c_client *client)	//lyq
{
	u8 databuf[2];
	int res = 0;
	...
	res = ap3xx6_init_als(client);
	res = ap3220_ps_init(client);
...
/*static int ap3xx6_get_OBJ(struct i2c_client *client)
{
	...
	printk("lyq:ps raw is %d,thd_val_h is %d\n",obj->ps,atomic_read(&obj->ps_thd_val_h));
...
static void ap3xx6_eint_work(struct work_struct *work)
{
	...
	if(err < 0)
	{
		...
		write_ps_ctr_reg(obj->client,0);	//close ps function	
		
		ps_value = ap3220_change_ps_threshold(obj->client);//ap3xx6_get_OBJ(obj->client);
		err = ps_report_interrupt_data(ps_value);
		LYQ_LOG("PS input value is %d\n",ps_value);
...
#if defined(CONFIG_OF)
	enable_irq(obj->irq);
	write_ps_ctr_reg(obj->client,1);	// open ps function	
#else
...
static long ap3xx6_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case NVRAM_IOCTL_SET_LIGHT:
		if(copy_from_user(ls_cali_temp, ptr, sizeof(ls_cali_temp)))
		{
			err = -EFAULT;
			goto err_out;
		}
		if(set_light_cail(ls_cali_temp,obj))
			break;
		else
			APS_ERR("%s set light cail is failure\n",__func__);
			break;


        case NVRAM_IOCTL_SET_PS:
		if(copy_from_user(&ps_cali_temp, ptr, sizeof(ps_cali_temp)))
		{
			err = -EFAULT;
			goto err_out;
		}
		if(set_ps_cail(ps_cali_temp, obj))
			break;
		else
			APS_ERR("%s set cail is failure\n",__func__);
		break;

	case IOCTL_GET_LIGHT_CAIL:
		if(copy_to_user(ptr,&obj->als_cail,sizeof(obj->als_cail)))
		{
			err= -EFAULT;
			APS_ERR("%s get light cail is failure\n",__func__);
			goto err_out;
		}
		break;

	case IOCTL_GET_PS_CAIL:
		if(copy_to_user(ptr,&obj->ps_cail,sizeof(&obj->als_cail)))
		{
			err= -EFAULT;
			APS_ERR("%s get ps cail is failure\n",__func__);
			goto err_out;
		}
		break;
...
static int ap3xx6_als_get_data(int *value, int *status)
{
...
#endif
	{
		...
		{
                       temp_als = temp_value;
                       LYQ_LOG("ap3220 light input value is %d, raw value is %d\n",temp_value, obj->als);
                }
	}
...
*/
	...
	err = ap3xx6_enable_ps(obj->client, 1);
		...
		ps_value = get_ps_level(obj->client,0xff);
...
#if defined(CONFIG_OF)
	obj->irq_node = of_find_compatible_node(NULL, NULL, "mediatek,alsps_int");
#endif
