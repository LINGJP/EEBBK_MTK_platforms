...
int blkdev_boot_read(blkdev_t *bdev, u64 src, u32 size, u8 *dst)
{
    ...
    if (part_start) {
        blknr = aligned_start / blksz;
        part_len = part_start + size > blksz ? blksz - part_start : size;
        if (mmc_bread_boot(bdev, (unsigned long)blknr, 1, (unsigned long*)buf) != 0)
            return -1;
        memcpy(dst, buf + part_start, part_len);
        dst  += part_len;
        src  += part_len;
    ...
    if (size >= blksz) {
        ...
        if (blks && 0 != mmc_bread_boot(bdev, (unsigned long)blknr, blks, (unsigned long*)dst))
            return -1;
    ...
    if (size && part_end && src < end) {
        blknr = aligned_end / blksz;
        if (mmc_bread_boot(bdev, (unsigned long)blknr, 1, (unsigned long*)buf))
            return -1;
        memcpy(dst, buf, part_end);
    }
    return 0;
