...
int nand_exec_read_page_hw_single(struct nand_chip *nand, u32 u4RowAddr, u32 u4PageSize, u8 *pPageBuf, u8 *pFDMBuf)
{
	int bRet;
...
int nand_exec_read_page_hw(struct nand_chip *nand, u32 u4RowAddr, u32 u4PageSize, u8 *pPageBuf, u8 *pFDMBuf)
{
	...
	if (devinfo.two_phyplane)
		pagesize >>= 1;
	bRet = nand_exec_read_page_hw_single(nand, u4RowAddr, pagesize, main_buf, spare_buf);
	if (bRet != ERR_RTN_SUCCESS)
		return bRet;
	if (devinfo.two_phyplane) {
		u4SecNum >>= 1;
		...
		bRet = nand_exec_read_page_hw_single(nand, u4RowAddr + page_per_block, pagesize, main_buf, spare_buf);
	}
	return bRet;
}
...
int nand_erase(u64 offset, u64 size)
{
	...
	while (tblksz != 0) {
		...
		if ((!raw_partition(index))
		        && (devinfo.NAND_FLASH_TYPE == NAND_FLASH_MLC_HYBER) && (!mtk_block_istlc(cur_offset))) {
...
bool __nand_erase(u64 logical_addr)
{
	...
	if (!nand_erase_hw((u64) mapped_block * BLOCK_SIZE)) {
...
#endif
{
	...
	printf("[nand_wite_img]write to addr %llx,img size %x\n", addr, img_sz);
	...
	printf("[nand_write_img]img_sz %x write_size %x addr %llx\n",img_sz,write_size, addr);
