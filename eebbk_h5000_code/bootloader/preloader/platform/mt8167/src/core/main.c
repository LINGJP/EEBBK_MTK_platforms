...
static void bldr_post_process(void)
{
    ...
    if(wdt_rpmb_program_mode() != false)
    {
        ...
        {
            printf("[RPMB] Begin RPMB provisioning\n");
            seclib_rpmb_mac_process_ex(mmc_rpmb_set_key, 0);
            platform_safe_mode(0, 0);
            mtk_arch_reset(0);
        } else {
            printf("[RPMB] In RPMB provisioning mode but RPMB already provisioned\n");
        }
#else
...
char* check_active_part(char *name1, char *name2)
{
    int ret;
    unsigned long name1_active = 0, name2_active = 0;

    print("%s check active part. of %s and %s\n", MOD, name1, name2);

    /* check the active bit of first partition */
    part_t *part = part_get(name1);
    if (NULL == part) {
        print("%s %s part. not found\n", MOD, name1);
        return -1;
    } else {
        name1_active = mt_part_get_active_bit(part);
    }

    /* check the active bit of second partition */
    part = part_get(name2);
    if (NULL == part) {
        print("%s %s part. not found\n", MOD, name2);
        /* return -1; Comment it since we could load first partition to boot */
    } else {
        name2_active = mt_part_get_active_bit(part);
    }

    print("%s %s active = %x, %s active = %x\n", MOD, name1, name1_active, name2, name2_active);

    if (name1_active == 0 && name2_active > 0) {
        print("%s Loading %s Partition...\n", MOD, name2);
        return name2;
    } else {
        print("%s Loading %s Partition...\n", MOD, name1);
        return name1;
...
int bldr_load_tee_part(char *name, blkdev_t *bdev, u32 *addr, u32 offset, u32 *size)
{
    ...
    if (ret) {
        print("%s %s part. ATF verify fail\n", MOD, name);
        return ret;
    }
    ...
    /* header addr will be updated to entry point addr */
    /* stiil need to decode mtee image */
...
#elif CFG_LOAD_UBOOT
    name = check_active_part("lk", "lk2");

    addr = CFG_UBOOT_MEMADDR;
    ret = bldr_load_part(name, bootdev, &addr, &size);
    if (ret) {
       addr = CFG_UBOOT_MEMADDR;
       ret = bldr_load_part("lk2", bootdev, &addr, &size);
       if (ret)
           return ret;
    }
    *jump_addr = addr;
...
#if CFG_ATF_SUPPORT
    name = check_active_part("tee1", "tee2");

    addr = CFG_ATF_ROM_MEMADDR;
    ret = bldr_load_tee_part(name, bootdev, &addr, 0, &size);
...
#if DDR_AUTO_REBOOT
		print("%s Autoreboot yifei\n", MOD);
		*(unsigned int*) 0x10007014 = 0x1209;
#endif
