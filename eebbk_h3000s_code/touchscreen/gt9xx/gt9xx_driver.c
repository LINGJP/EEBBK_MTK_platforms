...
u8 config_tp_cfg[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH]
...
#define GOODIX_I2C_SLAVE_ADDR  0x14
...
static unsigned short force[] = {0, GOODIX_I2C_SLAVE_ADDR, I2C_CLIENT_END, I2C_CLIENT_END};
...
static unsigned int tpd_id_gpio;
/*for main board id, has no things to tp, start*/
static int main_board_id_gpio1;
static int main_board_id_gpio2;
/*for main board id, has no things to tp, end*/
#endif
static int of_get_gt9xx_platform_data(struct device *dev)
{
	...
	if (dev->of_node) {
		...
		tpd_id_gpio = of_get_named_gpio(dev->of_node, "id-gpio", 0);

		GTP_INFO("g_vproc_en_gpio_number %d\n", tpd_rst_gpio);
		GTP_INFO("g_vproc_vsel_gpio_number %d\n", tpd_int_gpio);
		GTP_INFO("g_vproc_id_gpio_number %d\n", tpd_id_gpio);
		
		/*for main board id, has no things to tp, start*/
		main_board_id_gpio1 = of_get_named_gpio(dev->of_node, "mainboard_id-gpio1", 0);
		main_board_id_gpio2 = of_get_named_gpio(dev->of_node, "mainboard_id-gpio2", 0);
		GTP_INFO("main_board_id_gpio1 %d\n", main_board_id_gpio1);
		GTP_INFO("main_board_id_gpio2 %d\n", main_board_id_gpio2);
		/*for main board id, has no things to tp, end*/
	}
#endif
	return 0;
...
static ssize_t gt91xx_config_read_proc(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	...
	for (i = 0; i < GTP_CONFIG_MAX_LENGTH; i++) {
		ptr += sprintf(ptr, "0x%02X ", config_tp_cfg[i + 2]);
					...
					loff_t *ppos)
{
	...
	if (copy_from_user(&config_tp_cfg[2], buffer, count)) {
		...
		abs_x_max = (config_tp_cfg[RESOLUTION_LOC + 1] << 8) + config_tp_cfg[RESOLUTION_LOC];
		abs_y_max = (config_tp_cfg[RESOLUTION_LOC + 3] << 8) + config_tp_cfg[RESOLUTION_LOC + 2];
		int_type = (config_tp_cfg[TRIGGER_LOC]) & 0x03;
...
int i2c_read_bytes_non_dma(struct i2c_client *client, u16 addr, u8 *rxbuf, int len)
{
	...
	while (left > 0) {
		...
		while (ret != 2) {
			...
			if (retry == 5) {
				GTP_ERROR("I2C read 0x%X length=%d failed, ret=%d\n", addr + offset, len, ret);
				return -1;
			}
		}
	}
...
s32 gtp_send_cfg(struct i2c_client *client)
{
		...
		for (retry = 0; retry < 5; retry++) {
			ret = gtp_i2c_write(client, config_tp_cfg, GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);
			if (ret > 0)
				break;
...
s32 gtp_init_panel(struct i2c_client *client)
{
...
#else
	{
		...
		memset(&config_tp_cfg[GTP_ADDR_LENGTH], 0, GTP_CONFIG_MAX_LENGTH);
		memcpy(&config_tp_cfg[GTP_ADDR_LENGTH], send_cfg_buf[sensor_id], cfg_len);

#ifdef CONFIG_GTP_CUSTOM_CFG
		config_tp_cfg[RESOLUTION_LOC]		 = (u8)GTP_MAX_WIDTH;
		config_tp_cfg[RESOLUTION_LOC + 1] = (u8)(GTP_MAX_WIDTH>>8);
		config_tp_cfg[RESOLUTION_LOC + 2] = (u8)GTP_MAX_HEIGHT;
		config_tp_cfg[RESOLUTION_LOC + 3] = (u8)(GTP_MAX_HEIGHT>>8);

		if (GTP_INT_TRIGGER == 0)	/* RISING */
			config_tp_cfg[TRIGGER_LOC] &= 0xfe;
		else if (GTP_INT_TRIGGER == 1)	/* FALLING */
			config_tp_cfg[TRIGGER_LOC] |= 0x01;

#endif	/* CONFIG_GTP_CUSTOM_CFG */

		check_sum = 0;
		for (i = GTP_ADDR_LENGTH; i < cfg_len; i++)
			check_sum += config_tp_cfg[i];

		config_tp_cfg[cfg_len] = (~check_sum) + 1;
	...
	ret = gtp_i2c_read(client, config_tp_cfg, cfg_len + GTP_ADDR_LENGTH);
	if (ret < 0) {
	...
	if (CHIP_TYPE_GT9F == gtp_chip_type) {
		...
		driver_num = (config_tp_cfg[CFG_LOC_DRVA_NUM]&0x1F) + (config_tp_cfg[CFG_LOC_DRVB_NUM]&0x1F);
		sensor_num = (config_tp_cfg[CFG_LOC_SENS_NUM]&0x0F) + ((config_tp_cfg[CFG_LOC_SENS_NUM]>>4)&0x0F);
		have_key = config_tp_cfg[GTP_REG_HAVE_KEY - GTP_REG_CONFIG_DATA + 2] & 0x01;	/* have key or not */
		if (1 == have_key)
			driver_num--;
...
#endif
	{
...
#else
		{
			...
			if (flash_cfg_version < 90 && flash_cfg_version > drv_cfg_version) {
				config_tp_cfg[GTP_ADDR_LENGTH] = drv_cfg_version;
				check_sum = 0;
				for (i = GTP_ADDR_LENGTH; i < cfg_len; i++)
					check_sum += config_tp_cfg[i];
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
	...
	GTP_DEBUG("client->addr = 0x%x", client->addr);

	/* select client address */
	gtp_gpio_output(GTP_IRQ_GPIO, client->addr == GOODIX_I2C_SLAVE_ADDR);
...
reset_proc:
	...
	GTP_INFO("turn on power reg-vgp6\n");
...
static int tpd_irq_registration(void)
{
	...
	if (node) {
		...
		printk("%s, %dtouch_irq = %d, val = %d\n\n", __func__, __LINE__, touch_irq, irqf_val);
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("%s, %di2c_client_point = 0x%p, addr = 0x%x\n\n", __func__, __LINE__, i2c_client_point, i2c_client_point->addr);

	if (i2c_client_point->addr != GOODIX_I2C_SLAVE_ADDR)
	{
		printk("%s, %d============warning=========== 0x%x\n\n", __func__, __LINE__, i2c_client_point->addr);
		i2c_client_point->addr = GOODIX_I2C_SLAVE_ADDR;
	}
	...
	if (ret < 0) {
		GTP_ERROR("fatal error, I2C communication ERROR!");
		goto out;
	}
	...
	input_set_capability(tpd->dev, EV_KEY, GESTURE_KEY_RIGHT);
	input_set_capability(tpd->dev, EV_KEY, GESTURE_KEY_S);
	input_set_capability(tpd->dev, EV_KEY, GESTURE_KEY_V);
	input_set_capability(tpd->dev, EV_KEY, GESTURE_KEY_DOUBLE_CLICK);
#endif
...
#ifdef CONFIG_GTP_GESTURE_WAKEUP
   tp_sysfs_gesture_init();
#endif

	gtp_debug_sysfs_init();
	
	/*add by lmh tp module factory and vendor start*/
	gtp_sensor_id_init();	//模组厂
	gtp_vendor_init();		//原厂厂商
	gtp_configVersion_init();	//固件和参数版本号
	/*add by lmh tp vendor node for selftest end*/

	/*for main board id, has no things to tp, start*/
	gtp_main_board_id_init();	//机器主板，与tp无关
	/*for main board id, has no things to tp, end*/
	...
	if (on) {
		...
		if (!atomic_cmpxchg(&gtp_chr_running, 0, 1))
			queue_delayed_work(gtp_workqueue,
					&gtp_charger_check_work, clk_tick_cnt_charger);
	} else {
...
static int touch_event_handler(void *unused)
{
       ...
       wait_event_interruptible(waiter, tpd_flag != 0 && tpd_register_flag == 1);
       tpd_flag = 0;
       ...
       set_current_state(TASK_RUNNING);
		...
		//GTP_DEBUG("%s enter, line=%d", __func__, __LINE__);
#ifdef CONFIG_GTP_GESTURE_WAKEUP
		if (DOZE_ENABLED == gesture_data.doze_status) {
		...
		if (key_value || pre_key) {
			for (i = 0; i < TPD_KEY_COUNT; i++) {
				GTP_DEBUG("Touch the GTP key, key_value = %d", key_value);
...
#else
	{
		...
		while (retry++ < 5) {
			...
			if (ret > 0) {
				...
				ret = i2c_read_bytes(client, GTP_REG_SLEEP, (u8 *)&value, sizeof(value));
				GTP_INFO("value ==== %d\n", value);
				return ret;
			}
			msleep(20);
...
/*add by lmh tp module factory and vendor start*/
static ssize_t gtp_sysfs_sensorid_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	u8 sensorId = 0;
	int val = 0;
	
    gtp_i2c_read_dbl_check(i2c_client_point, GTP_REG_SENSOR_ID, &sensorId, 1);
	GTP_DEBUG("lmh --- sensor_id = %d", sensorId);
...
static ssize_t gtp_sysfs_sensorid_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
    return -EPERM;
}

static DEVICE_ATTR(sensorid, S_IRUGO|S_IWUSR, gtp_sysfs_sensorid_show, gtp_sysfs_sensorid_store);

//模组厂
int gtp_sensor_id_init()
{
	s32 ret ;
    ...
    ret = sysfs_create_file(tp_gesture_kobj, &dev_attr_sensorid.attr);
    if (ret)
    {
    ...
    GTP_INFO("Goodix sensorid sysfs create success!\n");
    return 0 ;
}


static ssize_t gtp_sysfs_vendor_show(struct device *dev,struct device_attribute *attr, char *buf)
{	
	int val = 0;
	int id_gpio_state = -1;

	id_gpio_state = gpio_get_value(tpd_id_gpio);

	if(id_gpio_state == 0){
		val = sprintf(buf, "goodix\n");
	} else{
		val = sprintf(buf, "unknow\n");
	}

	return val;
}

static ssize_t gtp_sysfs_vendor_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
    return -EPERM;
}

static DEVICE_ATTR(vendor, S_IRUGO|S_IWUSR, gtp_sysfs_vendor_show, gtp_sysfs_vendor_store);
...
int gtp_vendor_init()
{
	...
	gpio_direction_input(tpd_id_gpio);
    ...
    ret = sysfs_create_file(tp_gesture_kobj, &dev_attr_vendor.attr);
    if (ret)
    {
        GTP_ERROR("%s: sysfs_create_vendor_file failed\n", __func__);
        return ret;
    }

    GTP_INFO("Goodix vendor sysfs create success!\n");
    return 0 ;
}

static ssize_t gtp_sysfs_configVersion_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	//u8 configVersion[2] = {0};
	u8 configVersion = 0;
	u16 version_info = 0;
	int val = 0;
	
    gtp_i2c_read_dbl_check(i2c_client_point, GTP_REG_CONFIG_DATA, &configVersion, 1);
	gtp_read_version(i2c_client_point, &version_info);
	GTP_DEBUG("lmh --- firmware_version = %x,  parameter_version = 0X%x", version_info, configVersion);

	val = sprintf(buf, "firmware=%x, parameter=%x\n", version_info, configVersion);

	return val;
}

static ssize_t gtp_sysfs_configVersion_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
    return -EPERM;
}

static DEVICE_ATTR(configVersion, S_IRUGO|S_IWUSR, gtp_sysfs_configVersion_show, gtp_sysfs_configVersion_store);

//配置参数版本号
int gtp_configVersion_init()
{
	s32 ret ;
    ...
    ret = sysfs_create_file(tp_gesture_kobj, &dev_attr_configVersion.attr);
    if (ret)
    {
        GTP_ERROR("%s: sysfs_create_version_file failed\n", __func__);
        return ret;
    ...
    GTP_INFO("Goodix configVersion sysfs create success!\n");
    return 0 ;
}

/*add by lmh tp module factory and vendor end*/


/*for main board id, has no things to tp, start*/
static ssize_t gtp_sysfs_main_board_id_show(struct device *dev,struct device_attribute *attr, char *buf)
{	
	int val = 0;
	int main_board_id_gpio1_state = -1;
	int main_board_id_gpio2_state = -1;

	main_board_id_gpio1_state = gpio_get_value(main_board_id_gpio1);
	main_board_id_gpio2_state = gpio_get_value(main_board_id_gpio2);

	if(main_board_id_gpio1_state==0 && main_board_id_gpio2_state==0){
		val = sprintf(buf, "00\n");
	} else if(main_board_id_gpio1_state==0 && main_board_id_gpio2_state==1){
		val = sprintf(buf, "01\n");
	} else if(main_board_id_gpio1_state==1 && main_board_id_gpio2_state==0){
		val = sprintf(buf, "10\n");
	} else if(main_board_id_gpio1_state==1 && main_board_id_gpio2_state==1){
		val = sprintf(buf, "11\n");
	} else {
		val = sprintf(buf, "unknow\n");
	}
...
static ssize_t gtp_sysfs_main_board_id_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
    return -EPERM;
}

static DEVICE_ATTR(main_board_id, S_IRUGO|S_IWUSR, gtp_sysfs_main_board_id_show, gtp_sysfs_main_board_id_store);

//机器主板，与tp无关
int gtp_main_board_id_init()
{
	s32 ret ;

	if(main_board_id_gpio1 >= 0)
		gpio_direction_input(main_board_id_gpio1);

	if(main_board_id_gpio2 >= 0)
		gpio_direction_input(main_board_id_gpio2);
    ...
    ret = sysfs_create_file(tp_gesture_kobj, &dev_attr_main_board_id.attr);
    if (ret)
    {
        GTP_ERROR("%s: sysfs_create_main_board_id_file failed\n", __func__);
        return ret;
    }

    GTP_INFO("main_board_id sysfs create success!\n");
    return 0 ;
}
/*for main board id, has no things to tp, end*/
