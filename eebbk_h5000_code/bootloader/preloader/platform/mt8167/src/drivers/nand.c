...
bool slc_mode_en = FALSE;
bool two_phyplane = FALSE;
...
bool get_device_info(u8*id, flashdev_info *devinfo)
{
    ...
    if(target != -1){
		...
		two_phyplane = devinfo->two_phyplane;
		if (two_phyplane)
...
int mtk_nand_read_page_hw_single(u32 page, u8 * dat, u8 * oob)
{
	...
	do{
	    ...
	    if (mtk_nand_ready_for_read(real_row_addr, u4SecNum, pPageBuf))
	    {
			...
			if(0 != (DRV_Reg32(NFI_STA_REG32) & STA_READ_EMPTY))
			{
    		    ...
    		    {
    				memset(pPageBuf, 0xFF,u4SecNum << g_nand_chip.sector_shift);
...
int mtk_nand_read_page_hw(u32 page, u8 * dat, u8 * oob)
{
	...
	if (devinfo.two_phyplane)
		u4SecNum >>= 1;
	bRet = mtk_nand_read_page_hw_single(page, main_buf, spare_buf);
	if (bRet != ERR_RTN_SUCCESS)
		return bRet;
	if (devinfo.two_phyplane) {
		main_buf += u4SecNum << g_nand_chip.sector_shift;
		spare_buf += u4SecNum * 8;
		bRet = mtk_nand_read_page_hw_single(page + page_per_block, main_buf, spare_buf);
	}
	return bRet;
}
...
bool nand_block_bad_hw(u64 logical_addr)
{
    ...
    memset(g_nand_spare, 0x0, sizeof(g_nand_spare));
