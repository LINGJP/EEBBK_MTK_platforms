...
static int lcm_ldo_supply_enable_inno(void)
{
	int ret;
	unsigned int vol_vgp, vol_vmch;

	printk("lcm_ldo_supply_enable_inno\n");

	if (lcm_vgp_1v8 == NULL || lcm_vmch_3v3 == NULL)
		return 0;

	printk("LCM: set regulator voltage lcm_vgp_1v8 voltage to 1.8V\n");
	...
	printk("LCM: set regulator voltage lcm_vmch_3v3 voltage to 3.3V\n");
...
static int lcm_vgp_supply_disable_inno(void)
{
	int ret = 0;
	...
	printk("LCM: lcm query regulator enable status[0x%x, 0x%x]\n", is_vgp_enable, is_vmch_enable);
...
static void lcm_request_gpio_control(struct device *dev)
{
	...
	printk("lcd_id = %d, rst = %d, enp = %d, enn = %d\n", GPIO_LCD_ID, GPIO_LCD_RST, GPIO_LCD_ENP, GPIO_LCD_ENN);
}
...
static int lcm_platform_probe(struct platform_device *pdev)
{
	...
	if (!id) {
		printk("LCM: failed to of_match_node\n");
		return -ENODEV;
	}
...
static struct platform_driver lcm_driver = {
	...
	.driver = {
		   .name = "hx83100_wxga_dsi_vdo_innolux",
		   .owner = THIS_MODULE,
...
#else
#define GPIO_LCD_ENP		GPIO37
#endif

#ifdef GPIO_LCD_ENN
#define GPIO_LCD_ENN		GPIO_LCD_ENN
#else
#define GPIO_LCD_ENN		GPIO39
#endif
...
static void lcm_init_power(void)
{
	...
	LCM_LOGI("[LK/LCM] enp = %d, enn = %d, %d, %d\n", GPIO_LCD_ENP, GPIO_LCD_ENN, mt_get_gpio_in(GPIO_LCD_ENP), mt_get_gpio_in(GPIO_LCD_ENN) );
	...
	gpio_set_value(GPIO_LCD_ENP, GPIO_OUT_ONE);
	MDELAY(5);
	gpio_set_value(GPIO_LCD_ENN, GPIO_OUT_ONE);
	MDELAY(5);
	
	LCM_LOGI("[Kernel/LCM] enp = %d, enn = %d, %d, %d\n", GPIO_LCD_ENP, GPIO_LCD_ENN, gpio_get_value(GPIO_LCD_ENP), gpio_get_value(GPIO_LCD_ENN) );
...
static void lcm_suspend_power(void)
{
	...
	gpio_set_value(GPIO_LCD_ENP, GPIO_OUT_ZERO);
	MDELAY(5);
	gpio_set_value(GPIO_LCD_ENN, GPIO_OUT_ZERO);
	MDELAY(5);

	lcm_set_gpio_output(GPIO_LCD_RST, GPIO_OUT_ZERO);
	MDELAY(120);
	
	lcm_vgp_supply_disable_inno();
	MDELAY(10);
#endif
}
...
{
#if 1
	if (LCM_FACTORY_ID_INNOLUX == get_lcm_manufactor() )	
		return 1;
	else
		return 0;
#else	
...
LCM_DRIVER hx83100_wxga_dsi_vdo_innolux_lcm_drv = {
	.name = "hx83100_wxga_dsi_vdo_innolux",
