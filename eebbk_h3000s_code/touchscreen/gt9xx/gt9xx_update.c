...
#ifdef CONFIG_GTP_ARRAY_FW_UPDATE
...
static u8 gup_enter_update_judge(struct st_fw_head *fw_head)
{
		...
		(!memcmp(fw_head->pid, "91XX", 4))) {
			...
			if (fw_head->vid != update_msg.ic_fw_msg.vid) {
				GTP_INFO("Need enter update.");//hgc
				return SUCCESS;
			}
...
#if (defined(CONFIG_GTP_AUTO_UPDATE) && !defined(CONFIG_GTP_REQUEST_FW_UPDATE ) && !defined(CONFIG_GTP_ARRAY_FW_UPDATE ) || defined(CONFIG_GTP_AUTO_UPDATE_CFG))
...
#endif	//CONFIG_GTP_REQUEST_FW_UPDATE	
#endif  //CONFIG_GTP_ARRAY_FW_UPDATE
#else
      {
...
#elif defined(CONFIG_GTP_AUTO_UPDATE) && defined (CONFIG_GTP_ARRAY_FW_UPDATE)
    if (got_file_flag == HEADER_FW_READY)
    {
        if(SEEK_SET == set_or_end)
        {
...
#if (defined(CONFIG_GTP_AUTO_UPDATE) && defined(CONFIG_GTP_AUTO_UPDATE_CFG) && (defined(CONFIG_GTP_REQUEST_FW_UPDATE) || defined(CONFIG_GTP_ARRAY_FW_UPDATE) ) )
	if (NULL == dir) {
