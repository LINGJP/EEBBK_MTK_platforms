...
/*add by lmh start*/
static ssize_t fts_chip_id_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ssize_t num_read_chars = 0; 
    u8 chip_id = 0; 
    ...
    fts_esdcheck_proc_busy(1);
#endif
    if (fts_i2c_read_reg(fts_i2c_client, FTS_REG_CHIP_ID, &chip_id) < 0) 
    {    
        num_read_chars = snprintf(buf, PAGE_SIZE,"I2c transfer error!\n");
    }    
#if FTS_ESDCHECK_EN
...
static ssize_t fts_chip_id_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    /* place holder for future use */
    return -EPERM;
}
...
/*add by lmh start*/
static DEVICE_ATTR(fts_chip_id, S_IRUGO|S_IWUSR, fts_chip_id_show, fts_chip_id_store);
/*add by lmh end*/
