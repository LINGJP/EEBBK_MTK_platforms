...
static unsigned int main_board_id_gpio1;
static unsigned int main_board_id_gpio2;
...
/*add by lmh tp module factory and vendor start*/
	...
	gtp_configVersion_init();	//配置参数版本号
			...
			if (ret < 0) {
				...
				if ((esd_buf[0] == 0xAA) || (esd_buf[1] != 0xAA)) {
					...
					if ((chk_buf[0] == 0xAA) || (chk_buf[1] != 0xAA)) {
						GTP_DEBUG("ESD check：0x8040 error，need reset！");
...
static void tpd_down(s32 x, s32 y, s32 size, s32 id)
{
	...
	input_report_abs(tpd->dev, ABS_MT_POSITION_X, 799-y);
	...
	if (last_x != x || last_y != y)
	{
		last_x = x;
		last_y = y;
		GTP_INFO("down->id = %d, x,y = (%d, %d)", id, 799-y, x);
	}
