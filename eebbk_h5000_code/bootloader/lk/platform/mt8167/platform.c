...
void dump_boot_arg(void)
{
	...
	dprintf(CRITICAL,"draminfo: vendorid:%u size:%u type:%u\n", g_boot_arg->dram_info.dram_manid,g_boot_arg->dram_info.dram_size,g_boot_arg->dram_info.dram_type);
	dprintf(CRITICAL,"==dump boot argument==\n");
}
		...
		for (tags = (void *)BOOT_ARGUMENT_LOCATION; tags->hdr.size; tags = boot_tag_next(tags)) {
			switch (tags->hdr.tag) {
				...
				case BOOT_TAG_PL_VERSION:
					...
					break;
				case BOOT_TAG_DRAM_INFO:
					g_boot_arg->dram_info.dram_manid = tags->u.dram_info.dram_manid;
					g_boot_arg->dram_info.dram_size = tags->u.dram_info.dram_size;
				 	g_boot_arg->dram_info.dram_type = tags->u.dram_info.dram_type;
					break;
				default:
					break;
			}
...
//#ifndef MACH_FPGA
	...
	if (kernel_charging_boot() == 1) {
		...
		if ((g_boot_mode != LOW_POWER_OFF_CHARGING_BOOT) ||
			...
			//mt65xx_leds_brightness_set(MT65XX_LED_TYPE_LCD, 100);
#ifdef MTK_BATLOWV_NO_PANEL_ON_EARLY
		}
#endif
...
{
	int i=0, max_rank, target = -1;
	u64 start, end, sz, max_addr = 0;
	u64 reserved_addr = 0;

	if (reserved_size & (0x200000 - 1)) {
		dprintf(CRITICAL,"warning: reserved_size is not 2MB aligned\n");
		dprintf(CRITICAL,"mblock_info->mblock_num=%d, mblock[0].start0x%llx mblock_info->mblock_size=%llu\n",mblock_info->mblock_num,mblock_info->mblock[0].start,mblock_info->mblock[i].size);
	}
