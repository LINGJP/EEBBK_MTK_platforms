...
#define READ_RETRY_STEP devinfo.feature_set.FeatureSet.readRetryCnt//(devinfo.feature_set.FeatureSet.readRetryCnt + devinfo.feature_set.FeatureSet.readRetryStart)  // 8 step or 12 step to fix read retry table
...
#endif
	//printf("nand_exec_read_page_hw: page %d\n",u4RowAddr);
	/* MSG(INFO, "nand_exec_read_page_hw: 0x%x\n", u4RowAddr); */
	do {
...
//	int page_in_block = u4RowAddr % page_per_block;
	...
	printf("nand_exec_write_page_hw: page %d\n", u4RowAddr);
...
#if CFG_2CS_NAND
	if (g_bTricky_CS) {
		page_addr = mtk_nand_cs_on(NFI_TRICKY_CS, page_addr + mapped_block * page_per_block);
	}
#endif
...
//for tlc : just support slc mode block now
...
//	u32 mapped;
	...
	//printf("nand_part_write: dst 0x%llx\n", dst);
...
void nand_driver_test(void)
{
	...
	len = dev->read(dev, start_addr, (uchar *) original, test_len, NAND_PART_USER);
	if (len != test_len) {
	...
	len = dev->read(dev, start_addr, (uchar *) readback,test_len, NAND_PART_USER);
	if (len != test_len) {
...
// erase nand block
	...
	while (tblksz != 0) {
		...
		if ((!raw_partition(index))
		        && (devinfo.NAND_FLASH_TYPE == NAND_FLASH_TLC) && (!mtk_block_istlc(cur_offset))) {
