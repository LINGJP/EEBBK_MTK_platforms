...
static struct current_sensor_s *current_sensor =NULL;
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	memcpy((int *)buf,temp,sizeof(temp));
	return 3;
}
...
DEVICE_ATTR(alsdevnum,			  S_IRUGO, als_show_devnum,  NULL);
...
DEVICE_ATTR(psdevnum,			  S_IRUGO, ps_show_devnum,  NULL);
