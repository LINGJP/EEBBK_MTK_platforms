...
void dump_pmic_6392_reg(void)
{
	u32 i;
	U32 reg_val=0;

	print("dump_pmic_6392_reg\n");
	for (i=MT6392_CHR_CON0 ; i<=MT6392_CHR_CON7 ; i+=2) {
			pmic_read_interface(i,&reg_val,0xFFFF,0x0);
			print("[0x%x]=0x%x\n", i, reg_val);
	}
}
void hw_set_cc(int cc_val)
{
	...
	ret_val=pmic_config_interface(MT6392_CHR_CON1, 0x01, MT6392_PMIC_RG_VCDT_LV_VTH_MASK, MT6392_PMIC_RG_VCDT_LV_VTH_SHIFT);
	//VCDT_HV_EN=1
	...
	print("pl dump mt6392 reg\n");
	for (i=MT6392_CHR_CON0 ; i<=MT6392_CHR_CON29 ; i+=2) {
...
U32 pmic_init (void)
{
	...
	// eebbk <BBK_KEY_BSP> <20170411> <liudj> modify for long press pwrkey reboot begin
	ret_val = pmic_config_interface(MT6392_STRUP_CON13, 0, 0x3, 0);
	// eebbk <BBK_KEY_BSP> <20170411> <liudj> modify for long press pwrkey reboot end
...
#ifdef MTK_OZ105T_SUPPORT
	oz105t_hw_init();
	//oz105t_charging_enable(KAL_TRUE);
