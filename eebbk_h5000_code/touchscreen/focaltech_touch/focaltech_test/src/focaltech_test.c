...
#define FTS_RESULT_FILE_PATH  "/mnt/sdcard/"
/* modify by lmh start */
...
#endif
    nret = fts_test_entry_show("Conf_MultipleTest.ini");
	if (0 != nret)
    {
        num_read_chars = snprintf(buf, PAGE_SIZE, "tp test FAIL!\n");
    }
    else
    {
        num_read_chars = snprintf(buf, PAGE_SIZE, "tp test SUCCEED!\n");
    }
