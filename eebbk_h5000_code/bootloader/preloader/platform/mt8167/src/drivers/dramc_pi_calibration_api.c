...
void DramcSwImpedanceSaveRegister(DRAMC_CTX_T *p)
{
    ...
    U8 u1Offest_code_cmd_n = 0;
     ...
     //DQ
     //Drving B no use by Mason
     //vIO32WriteFldAlign(DDRPHY_SHU1_B0_DQ1, gDramcSwImpedanceResule[0]+u1Offest_code_b01_p, SHU1_B0_DQ1_RG_TX_ARDQ_DRVP_B_B0);
     //vIO32WriteFldAlign(DDRPHY_SHU1_B0_DQ1, gDramcSwImpedanceResule[1]+u1Offest_code_b01_n, SHU1_B0_DQ1_RG_TX_ARDQ_DRVN_B_B0);
     //vIO32WriteFldAlign(DDRPHY_SHU1_B0_DQ1+(1<<POS_BANK_NUM), gDramcSwImpedanceResule[0]+u1Offest_code_b23_p, SHU1_B0_DQ1_RG_TX_ARDQ_DRVP_B_B0);
     //vIO32WriteFldAlign(DDRPHY_SHU1_B0_DQ1+(1<<POS_BANK_NUM), gDramcSwImpedanceResule[1]+u1Offest_code_b23_n, SHU1_B0_DQ1_RG_TX_ARDQ_DRVN_B_B0);
     ...
     //CMD
     //Drving B no use by Mason
     //vIO32WriteFldAlign(DDRPHY_SHU1_CA_CMD1, gDramcSwImpedanceResule[0]+u1Offest_code_cmd_p, SHU1_CA_CMD1_RG_TX_ARCMD_DRVP_B);
     //vIO32WriteFldAlign(DDRPHY_SHU1_CA_CMD1, gDramcSwImpedanceResule[1]+u1Offest_code_cmd_n, SHU1_CA_CMD1_RG_TX_ARCMD_DRVN_B);
...
DRAM_STATUS_T DramcSwImpedanceCal(DRAMC_CTX_T *p)
{
	...
	if(g_Vendor_id == 0x3)
	{
		gDramcSwImpedanceResule[0] = 15;
		gDramcSwImpedanceResule[1] = 14;
	}
...
//    U32 u4O1Data[DQS_NUMBER];
    ...
    for (iDelay= iDelay_Start; iDelay <= iDelay_End; iDelay++)
    {
        if ((delayinecompare_flag & 1) == 0)
        {
            ...
            {    //Set CLK delay
                if (delayinecompare_flag & 2)
                {
                    vIO32WriteFldMulti(DDRPHY_SHU1_R0_CA_CMD10, P_Fld(-iDelay, SHU1_R0_CA_CMD10_RK0_TX_ARCLKB_DLYB)
                    						| P_Fld(-iDelay, SHU1_R0_CA_CMD10_RK0_TX_ARCLK_DLYB)
                    						| P_Fld(-iDelay, SHU1_R0_CA_CMD10_RK0_TX_ARCLKB_DLY)
                    						| P_Fld(-iDelay, SHU1_R0_CA_CMD10_RK0_TX_ARCLK_DLY));
                }
                else
                {
            ...
            {    // Set CA output delay
                if (delayinecompare_flag & 2)
                {
                    vIO32WriteFldAlign(DDRPHY_SHU1_R0_CA_CMD1, iDelay, SHU1_R0_CA_CMD1_RK0_TX_ARCMD1_DLY);
                    vIO32WriteFldAlign(DDRPHY_SHU1_R0_CA_CMD0+(1<<POS_BANK_NUM), iDelay, SHU1_R0_CA_CMD0_RK0_TX_ARCA6_DLY);
                    vIO32WriteFldAlign(DDRPHY_SHU1_R0_CA_CMD1, iDelay, SHU1_R0_CA_CMD1_RK0_TX_ARCMD2_DLY);
                    vIO32WriteFldAlign(DDRPHY_SHU1_R0_CA_CMD0, iDelay, SHU1_R0_CA_CMD0_RK0_TX_ARCA0_DLY);
                    vIO32WriteFldAlign(DDRPHY_SHU1_R0_CA_CMD1+(1<<POS_BANK_NUM), iDelay, SHU1_R0_CA_CMD1_RK0_TX_ARCMD2_DLY);
                    vIO32WriteFldAlign(DDRPHY_SHU1_R0_CA_CMD0+(1<<POS_BANK_NUM), iDelay, SHU1_R0_CA_CMD0_RK0_TX_ARCA7_DLY);
                    vIO32WriteFldAlign(DDRPHY_SHU1_R0_CA_CMD0, iDelay, SHU1_R0_CA_CMD0_RK0_TX_ARCA3_DLY);
                    vIO32WriteFldAlign(DDRPHY_SHU1_R0_CA_CMD0+(1<<POS_BANK_NUM), iDelay, SHU1_R0_CA_CMD0_RK0_TX_ARCA3_DLY);
                    vIO32WriteFldAlign(DDRPHY_SHU1_R0_CA_CMD0, iDelay, SHU1_R0_CA_CMD0_RK0_TX_ARCA4_DLY);
                    vIO32WriteFldAlign(DDRPHY_SHU1_R0_CA_CMD0+(1<<POS_BANK_NUM), iDelay, SHU1_R0_CA_CMD0_RK0_TX_ARCA0_DLY);
                }
                else
                {
    ...
    if ((delayinecompare_flag & 1) == 1)
    {
        if (delayinecompare_flag & 2)
        {
            vIO32WriteFldMulti(DDRPHY_SHU1_R0_CA_CMD10, P_Fld(0, SHU1_R0_CA_CMD10_RK0_TX_ARCLKB_DLYB)
            							| P_Fld(0, SHU1_R0_CA_CMD10_RK0_TX_ARCLK_DLYB)
            							| P_Fld(0, SHU1_R0_CA_CMD10_RK0_TX_ARCLKB_DLY)
            							| P_Fld(0, SHU1_R0_CA_CMD10_RK0_TX_ARCLK_DLY));
        }
        else
        {
...
void CATrainingDelayLineCali(DRAMC_CTX_T *p)
{
    U32 uiMR41;
    S32 iCenterSum = 0;
    S32 uiCA, uiCK_MAX;
    S32 iFirstCAPass[CATRAINING_NUM], iLastCAPass[CATRAINING_NUM], iCenter[CATRAINING_NUM];
    U32 iCA_PerBit_DelayLine[CATRAINING_NUM];
    ...
    CATrainingDelayCompare(p, uiMR41, u4GoldenPattern, &iCenterSum, iFirstCAPass, iLastCAPass, iCenter, 2);
    ...
    // Calculate golden setting¡F
    memset(DlyPerBit, 0, sizeof(DlyPerBit));
    for(ii=0; ii<CATRAINING_NUM; ii++) {
        DlyPerBit[ii].first_pass = iFirstCAPass[ii];
        DlyPerBit[ii].last_pass = iLastCAPass[ii];
        DlyPerBit[ii].win_size = iLastCAPass[ii] - iFirstCAPass[ii] + 1;
    }
}
...
DRAM_STATUS_T CATrainingLP3(DRAMC_CTX_T *p)
{
    ...
    U32 iCA_PerBit_DelayLine[CATRAINING_NUM];
    S32 iLastPassMin = 0xff, iCenterMin = 0xff, iCnt = 0, iFactor, iFactorMean = 0;
#endif
	...
	if (iFinalCACLK >= 0) {
		for(ii=0; ii<CATRAINING_NUM; ii++) {
			if (iLastPassMin > iLastCAPass[ii])
				iLastPassMin = iLastCAPass[ii];
			if (iCenterMin > iCenter[ii])
				iCenterMin = iCenter[ii];
		}
		iLastPassMin -= (iLastPassMin - iFinalCACLK) >> 1;
		if (iLastPassMin < 0)
			iLastPassMin = 0;
		vIO32WriteFldAlign(DDRPHY_SHU1_R0_CA_CMD9, 0 , SHU1_R0_CA_CMD9_RG_RK0_ARPI_CLK);
		vIO32WriteFldAlign(DDRPHY_SHU1_R0_CA_CMD9, 2, SHU1_R0_CA_CMD9_RG_RK0_ARPI_CS);
		vIO32WriteFldAlign(DDRPHY_SHU1_R0_CA_CMD9, iLastPassMin, SHU1_R0_CA_CMD9_RG_RK0_ARPI_CMD);

		CATrainingDelayLineCali(p);
		mcSHOW_DBG_MSG(("	PIEND	OFFSET	PISTEP	DLYSTEP	FACTOR\n"));
		for(ii=0; ii<CATRAINING_NUM; ii++) {
			if (iLastCAPass[ii]-iLastPassMin == 0)
				continue;
			iFactor = (DlyPerBit[ii].win_size - 1) * 1000 / (iLastCAPass[ii]-iLastPassMin);
			iFactorMean += iFactor;
			mcSHOW_DBG_MSG(("CA%d	%d	%d	%d	%d	%d\n", ii, iLastCAPass[ii], iLastPassMin, \
				iLastCAPass[ii]-iLastPassMin, DlyPerBit[ii].win_size - 1, iFactor));
			iCnt ++;
		}
		if (iCnt > 0)
			iFactorMean /= iCnt;
		...
		for (ii=0; ii<CATRAINING_NUM;ii++) {
			iCA_PerBit_DelayLine[ii] = ((iCenter[ii] - iCenterMin) * iFactorMean + 300) / 1000;
			mcSHOW_DBG_MSG(("CA%d perbit delay line = %d\n", ii, iCA_PerBit_DelayLine[ii]));
		}
		...
		for(ii=p->rank; ii<RANK_MAX; ii++)
		{
            ...
            if (p->pinmux == PIN_MUX_TYPE_LPDDR3_168BALL)
            {
