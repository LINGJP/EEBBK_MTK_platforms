...
int nand_read_page_bmt(u32 page, u8 * dat, u8 * oob)
{
	...
	return nand_exec_read_page_hw(nand_chip_bmt, row_addr, PAGE_SIZE_BMT, dat, oob);
...
#else
				MSG(INFO, "block[%d] map to block[%d]\n", bmt.table[i].bad_index, bmt.table[i].mapped_index);
#endif
			}
			free(phys_table);
#ifdef MNTL_SUPPORT
		memcpy((unsigned char *)ftl_bmt, dat_buf + MAIN_SIGNATURE_OFFSET+sizeof(phys_bmt_struct),
...
bmt_struct *init_bmt(struct nand_chip * chip, int size)
{
	...
	MSG(INFO, "bmt count: %d, system count: %d ppb: %d\n", bmt_block_count, system_block_count, page_per_block);
	pool_erased = 0;
