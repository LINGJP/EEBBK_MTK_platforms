...
static void mt_auxadc_clk_on(void)
{
	/*bit30=0: enable,bit30=1:disable*/
	*(volatile unsigned int *)AUXADC_CLK_CLR = *(volatile unsigned short *)AUXADC_CLK_SET | AUX_SW_CG_ADC;
}

static void mt_auxadc_clk_off(void)
{
	*(volatile unsigned int *)AUXADC_CLK_SET = *(volatile unsigned short *)AUXADC_CLK_SET | AUX_SW_CG_ADC;
}

static void mt_auxadc_power_on(void)
{
	*(volatile unsigned int *)AUXADC_MISC = *(volatile unsigned short *)AUXADC_MISC | (1 << 14);
}
...
int IMM_GetOneChannelValue(int dwChannel, int data[4], int* rawdata)
{
	...
	mt_auxadc_clk_on();
	//print("[adc_api]: auxadc clk(0x10000024=0x%x).\n", (*(volatile unsigned int *)AUXADC_CLK_STA));
	if ((*(volatile unsigned int *)AUXADC_CLK_STA) & AUX_SW_CG_ADC) {
		print("[adc_api]: auxadc clk(0x%x) is off.\n", (*(volatile unsigned int *)AUXADC_CLK_STA));
		return -1;
	}
   ...
   {
       print("[adc_api]: wait for module idle\n");
       udelay(10000);
	   idle_count++;
	   if(idle_count>30)
	   {
	      //wait for idle time out
	      print("[adc_api]: wait for aux/adc idle time out\n");
	      return -2;
	   }
   }
   ...
   {
       print("[adc_api]: wait for channel[%d] ready bit clear\n",dwChannel);
       udelay(10000);
	   data_ready_count++;
	   if(data_ready_count>30)
	   {
	      //wait for idle time out
	      print("[adc_api]: wait for channel[%d] ready bit clear time out\n",dwChannel);
	      return -3;
	   }
   }
   ...
   {
       print("[adc_api]: wait for channel[%d] ready bit ==1\n",dwChannel);
       udelay(10000);
	 data_ready_count++;
	 if(data_ready_count>30)
	 {
	      //wait for idle time out
	      print("[adc_api]: wait for channel[%d] data ready time out\n",dwChannel);
	      return -4;
	 }
   }
...
int IMM_GetOneChannelValue_Cali(int Channel, int*voltage)
{
     ...
     if(ret)
     {
	   ...
	   if(ret)
	   {
	        print("[adc_api]:IMM_GetOneChannelValue_Cali  get raw value error %d \n",ret);
		  return -1;
	   }
     }
...
static int IMM_auxadc_get_evrage_data(int times, int Channel)
{
	...
	while (i--)
	{
		...
		ret += ret_temp;
		print("[auxadc_get_data(channel%d)]: ret_temp=%d, ret = ret_value.\n",
...
int auxadc_test(void)
{
   ...
   //int Voltiage_cali =0;
	...
	for (i = 0; i < 16; i++) {
		//print("[adc_driver]: i=%d\n",i);
		res = IMM_GetOneChannelValue(i,data,&rawdata);
		if(res < 0) {
			   print("[adc_pl: get data error\n");
			   break;
		} else {
		       //print("[adc_pl channel[%d]raw =%d\n", i, rawdata);
		       //print("[adc_pl channel[%d]=%d.%.02d \n",i,data[0],data[1]);
		}
		#if 0
		res= IMM_GetOneChannelValue_Cali(i, &Voltiage_cali);
		if(res < 0) {
			print("[adc_driver]: get cali voltage error\n");
			break;
		} else {
			print("[adc_driver]: channel[%d] cali_voltage =%d\n", i, Voltiage_cali);
		}
		#endif
	}
   return 0;
