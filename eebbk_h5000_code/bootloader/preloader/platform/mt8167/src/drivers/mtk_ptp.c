...
static void ptp_init_det(struct ptp_det *det, struct ptp_devinfo *devinfo)
{
	...
	case PTP_DET_GPUSYS:
		...
		vcore_pmic = vcore_pmic - PTPOD_PMIC_OFFSET;
		if (det->VBOOT != vcore_pmic)
			ptp_error("VBOOT doesn't match. det->VBOOT = 0x%X, vcore_pmic = 0x%X\n", det->VBOOT, vcore_pmic);
...
static int set_vboot_to_pmic(struct ptp_det *det, unsigned int target_vboot)
{
	unsigned int vcore_pmic;

	ptp_notice("%s()\n", __func__);

	if (det->ctrl_id == PTP_CTRL_GPUSYS) {
		pmic_config_interface(0x312, target_vboot, 0x7F, 0);
		pmic_config_interface(0x314, target_vboot, 0x7F, 0);
...
void ptp_init(void)
{
	...
	set_vboot_to_pmic(&ptp_detectors[PTP_DET_GPUSYS], ptp_detectors[PTP_DET_GPUSYS].VBOOT + PTPOD_PMIC_OFFSET);
	...
	ptp_init_det(&ptp_detectors[PTP_DET_GPUSYS], &ptp_devinfo);
