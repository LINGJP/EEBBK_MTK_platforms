...
{
        //emmc/sdcard clock is on default.
...
void msdc_set_is_rpmb(struct mmc_host *host, int on)
{
	msdc_priv_t *priv = (msdc_priv_t*)host->priv;
...
void msdc_set_blkcount(struct mmc_host *host, u16 blockcount,
		u8 is_rel_write)
{
	int err;
	struct mmc_command cmd;
...
void msdc_config_clock(struct mmc_host *host, int ddr, u32 hz, u32 hs_timing)
{
    ...
    /*enable source clock cg after switch clock mode*/
    if (host->id == 0)
