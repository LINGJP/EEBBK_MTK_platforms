...
static int bq24196_iic_ok_test(void)
{
	...
	for (i=0; i<3; i++) {
		ret_code = bq24196_read_byte(0, &val);
		if (ret_code == I2C_OK)
			break;
	}
	...
	dprintf(CRITICAL, "lk bq24196 iic test = %d\n", bq24196_iic_test_ok);
...
//    if(upmu_get_cid() == 0x1020)
	...
	bq24196_set_iprechg(0x0); //Precharge current 128mA
...
#else
	bq24196_set_vreg(0x26); //VREG 4.112V
#endif
...
void bq24196_charging_enable(UINT8 bEnable)
{
	...
	if (STANDARD_CHARGER == CHR_Type_num) {
		...
		bq24196_set_ichg(0x2);  //Fast Charging Current Limit at 640mA
	} else {
    ...
    dprintf(CRITICAL,"[BATTERY:bq24196] charger enable = %d , charger type=%d!\r\n", bEnable, CHR_Type_num);
}
