...
static int lis3dh_suspend(struct device *dev,pm_message_t msg);
...
static void dumpReg(struct i2c_client *client)
{
	...
	for (i = 0; i < 14 ; i++) {
		...
		GSE_ERR("lyq:Reg addr=%x regdata=%x\n", addr, regdata);
		addr++;
	}
}
			...
			if (atomic_read(&priv->fir_en) && !atomic_read(&priv->suspend)) {
				...
				if (priv->fir.num < firlen) {
					...
					if (atomic_read(&priv->trace) & ADX_TRC_FILTER) {
						GSE_LOG("add [%2d] [%5d %5d %5d] => [%5d %5d %5d]\n", priv->fir.num, priv->fir.raw[priv->fir.num]
                            [LIS3DH_AXIS_X], priv->fir.raw[priv->fir.num][LIS3DH_AXIS_Y], priv->fir.raw[priv->fir.num][LIS3DH_AXIS_Z],
...
static int LIS3DH_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	if((databuf[0]&0x47) != 0x47)
	{
		databuf[0] = 0x47;
		GSE_ERR("lyq:%s error & reset lis3dh\n",__func__);
	}
...
static ssize_t show_status_value(struct device_driver *ddri, char *buf)
{
	...
	/*add by lyq to debug gsensor error start*/
static ssize_t show_em_value(struct device_driver *ddri, char *buf)
{
        struct lis3dh_i2c_data *obj = obj_i2c_data;
	int idx, count;
	u8 addr = 0x20;
	...
	for (idx = 0; idx < 14; idx++) {
		lis_i2c_read_block(obj_i2c_data->client, addr, &reg_value, 1);
                if (reg_value < 0) {
...
static ssize_t lis3dh_em_write(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	struct lis3dh_i2c_data *obj = obj_i2c_data;
        ...
        GSE_ERR("lyq:Reg[%x].Write [%x]..\n", addr, val);

	res = lis_i2c_write_block(obj->client, (u8)addr, (u8*)&val, 0x1);
        if (res < 0)
		GSE_ERR("lyq:lis_i2c_write_block error\n");
	else
		printk("lyq:lis_i2c_write_block ok\n");
        return count;
}
	/*add by lyq to debug gsensor error end*/
...
static DRIVER_ATTR(em,         S_IWUSR | S_IRUGO, show_em_value,        lis3dh_em_write);
...
#endif
static long lis3dh_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
static int lis3dh_enable_nodata(int en)
{
	...
	GSE_LOG("lis3dh_enable_nodata %d OK!\n", en);
	return 0;
}
...
static int lis3dh_get_data(int* x ,int* y,int* z, int* status)
{
	...
	*status = SENSOR_STATUS_ACCURACY_MEDIUM;
	...
	/*add by lyq for debug gsensor error start */	
	if(number==20)
	{
		number = 0;
		if((((*x)==data_temp[0][0]) && (data_temp[1][0]==data_temp[2][0]) && (data_temp[0][0]==data_temp[1][0]))
		|| (((*y)==data_temp[0][1]) && (data_temp[1][1]==data_temp[2][1]) && (data_temp[0][1]==data_temp[1][1]))
		|| (((*z)==data_temp[0][2]) && (data_temp[1][2]==data_temp[2][2]) && (data_temp[0][2]==data_temp[1][2])))
		{
			error_time++;
		}
		else
		...
		{
			error_time=0;
			GSE_ERR("lyq:lis3dh_get_data error_time is %d x %d y %d z %d\n",error_time,*x,*y,*z);
                        dumpReg(obj_i2c_data->client);
		}
	...
	number++;
	/*add by lyq for debug gsensor error end */	
	return 0;
}
