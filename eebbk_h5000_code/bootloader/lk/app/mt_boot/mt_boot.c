...
extern unsigned *target_atag_lyqbootreson(unsigned*ptr);
...
char g_boot_reason[][16]= {"power_key","usb","rtc","wdt","wdt_by_pass_pwk","tool_by_pass_pwk","2sec_reboot","kernel_panic","reboot","watchdog","kernel_restart","aee","recovery","bootloader","thermal","thermal_spm","spm","unknown"};
#if defined(MTK_SECURITY_SW_SUPPORT) && defined(MTK_VERIFIED_BOOT_SUPPORT)
...
#ifdef MTK_AB_OTA_UPDATER
/* FIXME: need to provide public key id and system partition path */
#define PUBLIC_KEY_ID "Android:#7e4333f9bba00adfe0ede979e28ed1920492b40f"
...
int bldr_load_dtb(char* boot_load_partition)
{
	...
	//load boot hdr
	ret = mboot_recovery_load_raw_part(part_name, ptr, sizeof(boot_img_hdr)+0x800);
	if (ret < 0) {
	...
	if (!g_is_64bit_kernel) {
		...
		ret = partition_read(part_name, (off_t)tmp, ptr, (size_t)DTB_MAX_SIZE);
		if (ret < 0) {
...
#define TMPBUF_SIZE 200
	...
	ptr = (char *)target_atag_lyqbootreson((unsigned *)buf);
	ret = fdt_setprop(fdt, offset, "atag,lyqbootreson", buf, ptr - buf);
	if (ret) return FALSE;
	else
	dprintf(CRITICAL,"atag,lyqbootreson set ok\n");
	...
	snprintf(tmpbuf, TMPBUF_SIZE, "rootwait ro init=/init root=/dev/dm-0 dm=\"system none ro,0 1 android-verity PARTUUID=%s %s\" androidboot.slot_suffix=%s", part->info->uuid, PUBLIC_KEY_ID,p_AB_suffix);
	cmdline_append(tmpbuf);
#ifdef USER_BUILD
	...
	dprintf(CRITICAL,"damon ----\n");
	
	if(g_boot_arg->dram_info.dram_type == 3)
	{
		char buf[200] = "";
		snprintf(buf,sizeof(buf)-1,"dram_venid=%u,dram_size=0x%x,dram_type=%u",g_boot_arg->dram_info.dram_manid,g_boot_arg->dram_info.dram_size,g_boot_arg->dram_info.dram_type);
		cmdline_append(buf);
		dprintf(CRITICAL,"damon ----xxxxxxx\n");
	}
	...
	if (clear_rtc_aldom_flag) {
		if (clear_rtc_aldom_flag()) {
			dprintf(CRITICAL,"clear RTC AL_DOM flag for kernel panic/restart/aee\n");
		}
	}
...
static void load_bootimg_by_suffix(void)
{
...
#endif
#ifdef LK_PROFILING
	unsigned int time_load_bootimg = get_timer(0);
#endif
	...
	if (AB_retry_count > 0) {
		...
		dprintf(CRITICAL, "[%s:%d] set_normal_boot(%s, %d): %d\n", __func__, __LINE__, p_AB_suffix, is_normal_boot, ret);
	}
...
#if defined(CFG_NAND_BOOT)
	snprintf(cmdline_tmpbuf, sizeof(cmdline_tmpbuf), "%s%x%s%x", NAND_MANF_CMDLINE, nand_flash_man_code, NAND_DEV_CMDLINE, nand_flash_dev_id);
...
#ifdef LK_PROFILING
	dprintf(CRITICAL,"[PROFILE] ------- load %s takes %d ms -------- \n", bootimg_name, (int)get_timer(time_load_bootimg));
#endif
}
