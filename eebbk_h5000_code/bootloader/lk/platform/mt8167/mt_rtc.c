...
bool clear_rtc_aldom_flag(void)
{
	U16 al_dom;

	/* use al_dom bit8~10 for kernel panic/restart/aee */
	al_dom = RTC_Read(RTC_AL_DOM);
	if ((al_dom & 0x0700) != 0x0) {
		dprintf(CRITICAL, "[RTC] al_dom = 0x%x\n", al_dom);
		rtc_writeif_unlock();
		RTC_Write(RTC_AL_DOM, al_dom & ~0x0700);
		rtc_write_trigger();
		return true;
	}
	return false;
