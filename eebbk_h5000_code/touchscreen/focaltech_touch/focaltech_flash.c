...
static int fts_ctpm_check_fw_status(struct i2c_client *client)
{
    ...
    for (i = 0; i < 5; i++)
    {
		...
		//add by lmh
        //if (( (chip_id1 == chip_types.chip_idh) || (chip_id1 == 0x54) )
        //if (( (chip_id1 == chip_types.chip_idh) || (chip_id1 == 0x54) || (chip_id1 == 0xef))
	...
	FTS_DEBUG("FTS_CHIP_IDC = %d", FTS_CHIP_IDC);
    ...
    if (i2c_noack_retry >= 5){
		FTS_INFO("i2c no ask 5 times, line=%d", __LINE__);
        return -EIO;
	}
    ...
    if (i >= 5)
    {
		FTS_INFO("i2c communication ok, but not get correct id, line=%d", __LINE__);
        fw_status = fts_ctpm_get_pram_or_rom_id(client);
    }

	FTS_INFO("fw_status = %d", fw_status);
    return fw_status;
}
