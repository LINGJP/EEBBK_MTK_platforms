...
#if CFG_ATF_SUPPORT
    tz_sec_mem_init((BL31_BASE&0xffff0000), (BL31_BASE&0xffff0000)+BL31_SIZE - 1, ATF_MPU_REGION_ID);
    DBG_MSG("%s set secure memory protection : 0x%x, 0x%x\n", MOD,
            (BL31_BASE&0xffff0000), (BL31_BASE&0xffff0000)+BL31_SIZE-1);
...
#elif CFG_MTK_IN_HOUSE_TEE_SUPPORT
    teearg->tee_support = 1;
...
{
    u32 bl31_reserve = (u32)ram_console_is_abnormal_boot(); /* Send boot status to ATF */
    tee_sec_config();
