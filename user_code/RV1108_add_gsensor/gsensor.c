...
struct gsensor sensor;
struct gsensor_send_data senddata[GSENSOR_FUN_NUM];
...
int gsensor_regsenddatafunc(int (*send)(struct sensor_axis data))
{
	...
	for (i = 0; i < GSENSOR_FUN_NUM; i++) {
		if (senddata[i].send == send) {						//lyq setup call function send
...
static int gsensor_readevent(struct sensor_axis *accdata, int count)
{
	...
	while (count) {
		...
		ret = poll(&sensor.fds, 1, 100);	//int this poll wait for gsensor data
		if (ret <= 0)
			continue;
		else
			...
			if (count <= 0) {
				...
				for (i = 0; i < GSENSOR_FUN_NUM; i++) {
					if ((senddata[i].send != NULL) &&
					    (senddata[i].num != 0)) {
						senddata[i].send(*accdata);		//lyq hui diao gsensor shuju
						send_count++;
					}
...
static void *gsensor_thread(void *arg)
{
	...
	while (!sensor.gsrelease_state) {					//lyq default value is 0 & 0 is enable gsensor & always read gsensor data
		...
		/* read gsensor data */
		ret = gsensor_readevent(&new_gs_data, 1);		//lyq read gsensor xyz data
		if (ret < 0)
			continue;
	}
...
int gsensor_init(void)
{
	...
	leavecarrec = parameter_get_leavecarrec();

	/* start or stop gsensor */
	if ((leavecarrec != 0) || (collision_level != 0)) {
		sensor.gsthread_state = 1;
		gsensor_create_thread();
	}
	pthread_mutex_unlock(&sensor.gs_mutex);
