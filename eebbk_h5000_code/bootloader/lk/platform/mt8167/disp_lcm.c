...
int _is_lcm_inited(disp_lcm_handle *plcm)
{
	if (plcm) {
		if (plcm->params && plcm->drv)
			...
			DISPMSG("plcm->params && plcm->drv !=1 so return not inited.\n");
	} else {
...
static DISP_MODULE_ENUM _get_dst_module_by_lcm(disp_lcm_handle *plcm)
{
	...
	if (plcm->params->type == LCM_TYPE_DSI) {
		...
		if (plcm->lcm_if_id == LCM_INTERFACE_DPI1)
...
#endif
			DISPCHECK
			("[LCM] vact: %d, vbp: %d, vfp: %d, vact_line: %d, hact: %d, hbp: %d, hfp: %d, hblank: %d, hblank: %d\n",
...
extern UINT32 DSI_dcs_read_lcm_reg_v2(DISP_MODULE_ENUM module, void *cmdq, UINT8 cmd,
...
void parse_lcm_params_dt_node(int node_offset, LCM_PARAMS *lcm_params)
{
	...
	disp_fdt_getprop_u32_array(node_offset, "lcm_params-dbi-data_format", &lcm_params->dbi.data_format);
	...
	disp_fdt_getprop_u32_array(node_offset, "lcm_params-dbi-serial-params1", &lcm_params->dbi.serial.cs_polarity);
	disp_fdt_getprop_u32_array(node_offset, "lcm_params-dbi-serial-params2", &lcm_params->dbi.serial.css);
	disp_fdt_getprop_u32_array(node_offset, "lcm_params-dbi-serial-params3", &lcm_params->dbi.serial.sif_1st_pol);
	disp_fdt_getprop_u32_array(node_offset, "lcm_params-dbi-serial-params4", &lcm_params->dbi.serial.sif_3wire);
	disp_fdt_getprop_u32_array(node_offset, "lcm_params-dbi-parallel-params", &lcm_params->dbi.parallel.write_setup);
	...
	disp_fdt_getprop_u32_array(node_offset, "lcm_params-dsi-data_format", &lcm_params->dsi.data_format);
	...
	disp_fdt_getprop_u32_array(node_offset, "lcm_params-dsi-ufoe_params", &lcm_params->dsi.ufoe_params);
	...
	disp_fdt_getprop_u8_array(node_offset, "lcm_params-dsi-lcm_esd_check_table0", &(lcm_params->dsi.lcm_esd_check_table[0]));
	disp_fdt_getprop_u8_array(node_offset, "lcm_params-dsi-lcm_esd_check_table1", &(lcm_params->dsi.lcm_esd_check_table[1]));
	disp_fdt_getprop_u8_array(node_offset, "lcm_params-dsi-lcm_esd_check_table2", &(lcm_params->dsi.lcm_esd_check_table[2]));
	...
	disp_fdt_getprop_u32_array(node_offset, "lcm_params-dsi-lane_swap", &lcm_params->dsi.lane_swap);
...
void load_lcm_resources_from_DT(LCM_DRIVER *lcm_drv)
{
	...
	offset = fdt_node_offset_by_compatible(g_fdt, -1, dts_params);
	...
	offset = fdt_node_offset_by_compatible(g_fdt, -1, dts_ops);
...
//ret 1 inno, 0 boe
int get_lcm_type(void)
{
    int ret1 = 0, ret2 = 0, lcd_id = 0;

    mt_set_gpio_mode(GPIO2, GPIO_MODE_00);
    mt_set_gpio_dir(GPIO2, GPIO_DIR_IN);
    mt_set_gpio_mode(GPIO73, GPIO_MODE_00);
    mt_set_gpio_dir(GPIO73, GPIO_DIR_IN);

    ret1 = mt_get_gpio_in(GPIO2);
    ret2 = mt_get_gpio_in(GPIO73);
    DISPMSG("lcm_gpio2,73 = %d, %d\n", ret1, ret2);

    if ( ( ret1 == 1) && (ret2 == 1) )
        lcd_id = 0; //35521s nova power ic on lcm boe
    else if ( ( ret1 == 1) && (ret2 == 0) )
        lcd_id = 0; //nova power ic on board boe
    else if ( ( ret1 == 0) && (ret2 == 0) )
        lcd_id = 1; //9365 fiti power ic on board inno
    else if ( ( ret1 == 0) && (ret2 == 1) )
        lcd_id = 2; //9364 fiti power ic on board boe
    else
        DISPERR("lcm type error!!!\n");
...
disp_lcm_handle *disp_lcm_probe(char *plcm_name, LCM_INTERFACE_ID lcm_id)
{
	...
	if (_lcm_count() == 0) {
		...
		isLCMFound = true;
	} else if (_lcm_count() == 3) {
        if (get_lcm_type() == 1)
            lcm_drv = lcm_driver_list[1]; //inno fiti
        else if (get_lcm_type() == 2)
            lcm_drv = lcm_driver_list[2]; //boe  fiti
        else if (get_lcm_type() == 0)
            lcm_drv = lcm_driver_list[0]; //boe  nova
        else
            lcm_drv = lcm_driver_list[0];
		...
		if (plcm_name == NULL) {
			...
			for (i = 0; i < _lcm_count(); i++) {
				...
				if (lcm_id == LCM_INTERFACE_NOTDEFINED
					...
					if (handle == NULL) {
						DISPERR
						("_display_interface_path_init returns NULL\n");
						goto FAIL;
					}
...
int disp_lcm_init(disp_lcm_handle *plcm)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (LCM_TYPE_DSI == plcm->params->type) {
			...
			if (ret == 0) {
				isLCMConnected = 0;
				DISPCHECK("lcm is not connected\n");
			} else {
				isLCMConnected = 1;
				DISPCHECK("lcm is connected\n");
			}
		} else if (LCM_TYPE_DPI == plcm->params->type) {
			isLCMConnected = 1;
		}
...
int disp_lcm_is_video_mode(disp_lcm_handle *plcm)
{
	...
	if (_is_lcm_inited(plcm))
		...
		lcm_param = plcm->params;
		DISPMSG("_is_lcm_inited=0; is_video_mode lcm_param->type=%d lcm_param->dsi.mode=%d\n", lcm_param->type, lcm_param->dsi.mode);
		//ASSERT(0);
	}
