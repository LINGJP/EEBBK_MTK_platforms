...
int set_ps_cail(unsigned int *p, struct ap3xx6_priv *obj)
{
	...
	if(flag == 1)
	{
		...
		if(temp > 12)
			ps_adjust_data = SAMPLE_PS_FACE + 20;
		else if(temp < 8)
			ps_adjust_data = SAMPLE_PS_FACE - 20;
		else
			ps_adjust_data = SAMPLE_PS_FACE;
...
/*		if (ps_adjust_data > STANDARD_PS_HOR_VALUE)
		...
		DBG_AP3220("%s,line=%d, temp is %d,ap3220_ps_threshole[0]=%d,[1]=%d,[2]=%d,[3]=%d,[4]=%d\n", __func__, __LINE__,temp, ap3220_ps_threshole[0], ap3220_ps_threshole[1], ap3220_ps_threshole[2], ap3220_ps_threshole[3], ap3220_ps_threshole[4]);
	}	
	else
		return -1;	
