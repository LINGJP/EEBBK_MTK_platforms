...
int nand_read_page_bmt(u32 page, u8 *dat, u8 *oob)
{
    ...
    return mtk_nand_read_page_hw(row_addr, dat, oob);
}
...
//MSG(INIT, "[%s]0x%x,0x%x,0x%x,0x%x \n",__FUNCTION__, *((UINT32 *)dat),*(((UINT32 *)dat)+1),*(((UINT32 *)dat)+2),*(((UINT32 *)dat)+3));
		MSG(INIT, "main sig not match\n");
        return false;
    }
    
    if (memcmp(oob + OOB_SIGNATURE_OFFSET, OOB_SIGNATURE, SIGNATURE_SIZE))
    {
        MSG(INIT, "main sig match, oob sig not match, but ignore\n");
    }
    return true;
}
...
static bool valid_bmt_data(phys_bmt_struct *phys_table)
{
    ...
    for (i = 0; i < phys_table->header.mapped_count; i++)
    {
        ...
        {
            MSG(INIT, "idx err: bad_idx: %d, mapped_idx: %d\n", 
...
static void fill_nand_bmt_buffer(bmt_struct *bmt, u8 *dat, u8 *oob)
{
	...
	memcpy(dat + MAIN_SIGNATURE_OFFSET+sizeof(phys_bmt_struct),
			(unsigned char *)&bmt->ftl_bmt, sizeof(struct ftl_bmt_struct));
	MSG(INIT, "FTL bmt found ftl_bmt->bad_count:%d\n", bmt->ftl_bmt.bad_count);
#endif
}
...
static int load_bmt_data(int start, int pool_size)
{
    ...
    int i;
#ifdef MNTL_SUPPORT
	struct ftl_bmt_struct *ftl_bmt = &bmt.ftl_bmt;

	memset((unsigned char *)ftl_bmt, 0, sizeof(struct ftl_bmt_struct));
#endif
#ifdef DBG_PRELOADER 
    MSG(INIT,  "[%s]: begin to search BMT from block 0x%x\n", __FUNCTION__, bmt_index);
#endif
	MSG(INIT,  "[%s]: bmt_idx=0x%x, start=0x%x, pool_size=0x%x,  \n", __FUNCTION__, bmt_index,start,pool_size);	

    for (bmt_index = start + pool_size - 1; bmt_index >= start; bmt_index--)
    {
MSG(INIT,  "[%s]: bmt_index=0x%x-- \n", __FUNCTION__, bmt_index);
 ...
 memset(oob_buf, 0xAA, 128);
        ...
        {
            MSG(INIT, "Err found when read block %d\n", bmt_index);
            continue;
        }
        if (!match_bmt_signature(bmt_buf, oob_buf))
        {
MSG(INIT,  "match_bmt_sig  out! \n"); 
            continue;
        }
		...
		#else
		MSG(INIT, "block %d is mapped to %d\n", bmt.table[i].bad_index, bmt.table[i].mapped_index);
#endif
            }
#ifdef MNTL_SUPPORT
		memcpy((unsigned char *)ftl_bmt, bmt_buf + MAIN_SIGNATURE_OFFSET+sizeof(phys_bmt_struct),
				sizeof(struct ftl_bmt_struct));
		MSG(INIT, "FTL bmt found ftl_bmt->bad_count:%d\n", ftl_bmt->bad_count);
		for (i = 0; i < ftl_bmt->bad_count; i++) {
		MSG(INIT, "bad_index:0x%x, flag:%d\n", ftl_bmt->entry[i].bad_index, ftl_bmt->entry[i].flag);
		}
#endif
            return bmt_index;
        }
...
print("sbc %d, tbc %d, ppb %d bbc %d\n",system_block_count,total_block_count,page_per_block,bmt_block_count);
#ifdef DBG_PRELOADER
