...
#define GESTURE_KEY_RIGHT               704
#define GESTURE_KEY_S                   712
#define GESTURE_KEY_V                   713
#define GESTURE_KEY_DOUBLE_CLICK        715
...
/* Log define */
#define GTP_INFO(fmt, arg...)           pr_err("GTP "fmt"\n", ##arg)
#define GTP_ERROR(fmt, arg...)          pr_err("GTP "fmt"\n", ##arg)
...
extern struct kobject *tp_gesture_kobj;
/*add by lmh tp module factory and vendor start*/
int gtp_sensor_id_init(void);
int gtp_vendor_init(void);
int gtp_configVersion_init(void);	//配置参数版本号
/*add by lmh tp module factory and vendor end*/

/*for main board id, has no things to tp, start*/
int gtp_main_board_id_init(void);
...
extern int gtp_debug_sysfs_init(void);
extern u8 config_tp_cfg[GTP_ADDR_LENGTH + GTP_CONFIG_MAX_LENGTH];
