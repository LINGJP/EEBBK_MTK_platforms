...
static int lcm_driver_probe(struct device *dev, void const *data)
{
    ...
    if ( (__gpio_get_value(GPIO_LCD_ID) == 0) && (__gpio_get_value(GPIO_LCD_ID2) == 1) ) {
        printk("it's jd9364 ic boe lcm");
    } else {
        printk("it's not jd9364 ic boe lcm");
        return -1;
    }
...
static const struct of_device_id lcm_platform_of_match[] = {
	{
		.compatible = "fiti,jd9364_wxga_dsi",
		.data = 0,
	}, {
		/* sentinel */
	}
...
static struct platform_driver lcm_driver = {
	...
	.driver = {
		   .name = "fiti_jd9364_wxga_dsi_vdo_boe",
		   .owner = THIS_MODULE,
...
LCM_DRIVER fiti_jd9364_wxga_dsi_vdo_boe_lcm_drv = {
	.name = "fiti_jd9364_wxga_dsi_vdo_boe",
