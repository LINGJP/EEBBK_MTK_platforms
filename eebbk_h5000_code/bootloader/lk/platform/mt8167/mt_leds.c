...
static int mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
  ...
  #endif
  	LEDS_DEBUG("cust->mode is %d\n",cust->mode);
	switch (cust->mode) {
		case MT65XX_LED_MODE_PWM:
			LEDS_DEBUG("cust->mode A\n");
			...
			LEDS_DEBUG("cust->mode A-1\n");
			if(strcmp(cust->name,"lcd-backlight") == 0)
			{
				if (BacklightLevelSupport == BACKLIGHT_LEVEL_PWM_256_SUPPORT){
					LEDS_DEBUG("cust->mode A-1a\n");
					level = brightness_mapping(level);
				}
				else{
					LEDS_DEBUG("cust->mode A-1b\n");
					level = brightness_mapto64(level);
				}
				LEDS_DEBUG("cust->mode A-2\n");

			    return brightness_set_pwm(cust->data, level,&cust->config_data);
			}
			else
			{
				LEDS_DEBUG("cust->mode A-1c\n");
				return led_set_pwm(cust->data, level);
			}
		case MT65XX_LED_MODE_GPIO:
			LEDS_DEBUG("cust->mode B\n");
			return ((cust_brightness_set)(cust->data))(level);
		case MT65XX_LED_MODE_PMIC:
			LEDS_DEBUG("cust->mode C\n");
			return brightness_set_pmic(cust->data, level);
		case MT65XX_LED_MODE_CUST_LCM:
			LEDS_DEBUG("cust->mode D\n");
			return ((cust_brightness_set)(cust->data))(level);
		case MT65XX_LED_MODE_CUST_BLS_PWM:
			LEDS_DEBUG("cust->mode E cust_data= 0x%x; level =%d\n", (cust->data), level);
			return ((cust_brightness_set)(cust->data))(level);
		case MT65XX_LED_MODE_NONE:
			LEDS_DEBUG("cust->mode F\n");
			break;
		default:
			LEDS_DEBUG("cust->mode G\n");
			break;
	}
	return -1;
}
...
//		level = 0;
	...
	LEDS_INFO("hgc->%s, %d, lastlevel = %d, level = %d\n", __func__, __LINE__, g_lastlevel[type], level);
...
void mt65xx_backlight_on_cust(int level)
{
	enum led_brightness backlight_level = level;
	LEDS_INFO("[LEDS]LK: mt65xx_backlight_on_cust:level =  %d\n\r",backlight_level);
	mt65xx_leds_brightness_set(MT65XX_LED_TYPE_LCD, backlight_level);
}
