...
//lmh add for esd begin
extern unsigned int touch_irq;
static u8 DOZE_INT_TYPE;
//lmh add for esd end
...
s32 gesture_event_handler(struct input_dev * dev)
{
	...
	if (DOZE_ENABLED == gesture_data.doze_status) {
		...
		if (ret == 0 && doze_buf[0] != 0 && doze_buf[0] != 0x81) {
		#if 1
			if (0xCC == doze_buf[0]) {
				...
				if (get_tp_gesture_flag() == 1)
				{
					input_report_key(dev, GESTURE_KEY_DOUBLE_CLICK, 1);
					input_sync(dev);
					input_report_key(dev, GESTURE_KEY_DOUBLE_CLICK, 0);
					input_sync(dev);
				}
				else
				{
				...
				return 1;
			} else if (0xAA == doze_buf[0]) {
				GTP_INFO("RIGHT slide to light up the screen!");
				doze_buf[0] = 0x00;
				ges_i2c_write_bytes(GTP_REG_WAKEUP_GESTURE, doze_buf, 1);

				if (get_tp_gesture_flag() == 1)
				{
					input_report_key(dev, GESTURE_KEY_RIGHT, 1);
					input_sync(dev);
					input_report_key(dev, GESTURE_KEY_RIGHT, 0);
					input_sync(dev);
				}
				else
				{
				...
				return 1;
			} else if ( ('s' == doze_buf[0]) || (0x3e == doze_buf[0])) {
				GTP_INFO("Wakeup by gesture(s), light up the screen!");
				doze_buf[0] = 0x00;
				ges_i2c_write_bytes(GTP_REG_WAKEUP_GESTURE, doze_buf, 1);

				if (get_tp_gesture_flag() == 1)
				{
					input_report_key(dev, GESTURE_KEY_S, 1);
					input_sync(dev);
					input_report_key(dev, GESTURE_KEY_S, 0);
					input_sync(dev);
				}
				else
				{
				...
				return 1;
			} else if ( ('v' == doze_buf[0]) || (0x76 == doze_buf[0])) {
				GTP_INFO("Wakeup by gesture(v), light up the screen!");
				doze_buf[0] = 0x00;
				ges_i2c_write_bytes(GTP_REG_WAKEUP_GESTURE, doze_buf, 1);

				if (get_tp_gesture_flag() == 1)
				{
					input_report_key(dev, GESTURE_KEY_V, 1);
					input_sync(dev);
					input_report_key(dev, GESTURE_KEY_V, 0);
					input_sync(dev);
				}
				else
				{
				...
				GTP_INFO("hgc->warning, other gesture !!!");
				doze_buf[0] = 0x00;
				ges_i2c_write_bytes(GTP_REG_WAKEUP_GESTURE, doze_buf, 1);
		...
		/*lmh add for esd begin*/
            ...
            //GTP_DEBUG(" invalid gesture int coming");
            //if (doze_buf[0]==0) {
            if (doze_buf[0]==0 || doze_buf[0]==0x81) {
                DOZE_INT_TYPE++;     // 1:first normal init after entering doze; 2: abnormal esd int
                GTP_DEBUG("DOZE_INT_TYPE = 0x%02X", DOZE_INT_TYPE);
    
                //if((DOZE_INT_TYPE != 1) && (doze_buf[1]==0) && (doze_buf[2]==0) && (doze_buf[3]==0) )  //abnoamal esd int
                if((DOZE_INT_TYPE != 1) && ( (doze_buf[1]==0) || (doze_buf[1]==0x81) ) && ( (doze_buf[2]==0) || (doze_buf[2]==0x81) ) && ( (doze_buf[3]==0) || (doze_buf[3]==0x81) ) )  
                {   
                    GTP_DEBUG("ESD need Reset");
                    //force reset:avdd--avdd--reset
                    GTP_DEBUG("force reset");
                    force_reset_guitar();
                    msleep(50);
                    ...
                    msleep(58);
                    //re-enter doze from suspend end
                    return 1;
                } else {
                    GTP_DEBUG(" first int after enter doze");
                    return 1;  //INT after enter doze 
                }   
            } else {
                return 1; /* doze enabled, but no invalid gesutre data*/
            }   
