...
/*mmc_switch_pll is workaround function
  mt8167 should gate clock when switching emmc/sdcard pll.
  */
static void mmc_config_pll(void)
{
	...
	/*set sdcard clock infra gate off*/
	DRV_SetReg32(MSDC_GATE_INFRA_SW_CG, MSDC1_INFRA_SW_CG);
	/*set sdcard clock gated off*/
	DRV_SetReg32(MSDC_CLK_GATE_CTRL, MSDC1_CLK_SW_CG);
	/* switch sdcard pll to 188M*/
	clk_sel = DRV_Reg32(MSDC_CLK_MUX_REG);
	clk_sel &= ~MSDC1_CLK_MUX_SEL;
	clk_sel |= (2 << 20);
	DRV_WriteReg32(MSDC_CLK_MUX_REG, clk_sel);
	/*clear sdcard clock gated off*/
	DRV_SetReg32(MSDC_CLK_UNGATE_CTRL, MSDC1_CLK_SW_CG);
	/*clear sdcard clock infra gate off*/
	DRV_SetReg32(MSDC_UNGATE_INFRA_SW_CG, MSDC1_INFRA_SW_CG);
}
...
extern U32 g_Density;
extern U32 g_dram_type;
...
void platform_set_boot_args_by_atag(unsigned *ptr)
{
	...
	tags = ptr;
	tags->hdr.size = boot_tag_size(boot_tag_dram_info);
	tags->hdr.tag = BOOT_TAG_DRAM_INFO;
	tags->u.dram_info.dram_manid = g_Vendor_id;
	tags->u.dram_info.dram_size  = g_Density;
	tags->u.dram_info.dram_type = g_dram_type;
	ptr += tags->hdr.size;
	print("set vendorid %u,dram_size %u,dram_type %u\n",g_Vendor_id,g_Density,g_dram_type);
...
#define MP0_DBG_CTRL 0x10200104
...
void platform_set_boot_args(void)
{
    ...
    *(volatile unsigned int *)MCU_ALL_PWR_ON_CTRL &= ~4;
    *(volatile unsigned int *)MCU_ALL_PWR_ON_CTRL &= ~2;
...
void charger_reg_rst(void)
{
#ifdef MTK_BQ24296_SUPPORT
	bq24296_set_reg_rst(0x1);
#endif
}
...
//#endif
	...
	//reset Charger IC to avoid system HW shutdown
	charger_reg_rst();
...
//enable long press reboot function***************
			...
			print("Long Pressed setting\n");
			pmic_config_interface(MT6392_TOP_RST_MISC, 0x01, MT6392_PMIC_RG_PWRKEY_RST_EN_MASK, MT6392_PMIC_RG_PWRKEY_RST_EN_SHIFT);
			pmic_config_interface(MT6392_TOP_RST_MISC, 0x00, MT6392_PMIC_RG_HOMEKEY_RST_EN_MASK, MT6392_PMIC_RG_HOMEKEY_RST_EN_SHIFT);
			pmic_config_interface(MT6392_STRUP_CON3,0x01, MT6392_PMIC_RG_FCHR_PU_EN_MASK, MT6392_PMIC_RG_FCHR_PU_EN_SHIFT);//pull up homekey pin of PMIC for 82 project
			pmic_config_interface(MT6392_TOP_RST_MISC, (U32)KPD_PMIC_LPRST_TD, MT6392_PMIC_RG_PWRKEY_RST_TD_MASK, MT6392_PMIC_RG_PWRKEY_RST_TD_SHIFT);
	#else
			pmic_config_interface(MT6392_TOP_RST_MISC, 0x01, MT6392_PMIC_RG_PWRKEY_RST_EN_MASK, MT6392_PMIC_RG_PWRKEY_RST_EN_SHIFT);
			pmic_config_interface(MT6392_TOP_RST_MISC, 0x01, MT6392_PMIC_RG_HOMEKEY_RST_EN_MASK, MT6392_PMIC_RG_HOMEKEY_RST_EN_SHIFT);

			pmic_config_interface(MT6392_STRUP_CON3,0x01, MT6392_PMIC_RG_FCHR_PU_EN_MASK, MT6392_PMIC_RG_FCHR_PU_EN_SHIFT);//pull up homekey pin of PMIC for 82 project
			pmic_config_interface(MT6392_TOP_RST_MISC, (U32)KPD_PMIC_LPRST_TD, MT6392_PMIC_RG_PWRKEY_RST_TD_MASK, MT6392_PMIC_RG_PWRKEY_RST_TD_SHIFT);
	#endif
#else
			pmic_config_interface(MT6392_TOP_RST_MISC, 0x00, MT6392_PMIC_RG_PWRKEY_RST_EN_MASK, MT6392_PMIC_RG_PWRKEY_RST_EN_SHIFT);
			pmic_config_interface(MT6392_TOP_RST_MISC, 0x00, MT6392_PMIC_RG_HOMEKEY_RST_EN_MASK, MT6392_PMIC_RG_HOMEKEY_RST_EN_SHIFT);
			pmic_config_interface(MT6392_STRUP_CON3,0x01, MT6392_PMIC_RG_FCHR_PU_EN_MASK, MT6392_PMIC_RG_FCHR_PU_EN_SHIFT);//pull up homekey pin of PMIC for 82 project
#endif
#else
...
void platform_init(void)
{
    ...
    if (reason == BR_RTC || reason == BR_REBOOT_RECOVERY || reason == BR_POWER_KEY || reason == BR_USB || reason == BR_WDT || reason == BR_WDT_BY_PASS_PWK || reason == BR_2SEC_REBOOT)
        rtc_bbpu_power_on();
#else
    ...
    mt_xo_init();
    BOOTING_TIME_PROFILING_LOG("XO initial");
...
/*In FPGA phase, dram related function should by pass*/
    ...
    bootarg.orig_dram_info.rank_info[1].size = 0x0FE00000;
